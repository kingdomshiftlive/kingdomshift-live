...
class MediaPickerHelper {
  ...
  Future<XFile?> pickImage({required ImageSource source}) async {
    try {
      return await _picker.pickImage(
          maxHeight: AppRes.maxHeight, maxWidth: AppRes.maxWidth, imageQuality: AppRes.imageQuality, source: source);
  ...
  Future<MediaFile?> pickVideo({required ImageSource source}) async {
    try {
      ...
      return MediaFile(file: videoFile, type: MediaType.video, thumbNail: thumbNail);
    } on PlatformException catch (e) {
      if (e.code == 'camera_access_denied') {
        _requestImagePermission(source);
      }
    ...
    if (file != null) {
      ...
      if (mimeType != null) {
        if (mimeType.contains('image')) {
          ...
          return MediaFile(type: MediaType.video, file: file, thumbNail: thumbNail);
        }
      } else {
  ...
  Future<XFile> extractThumbnail({required String videoPath}) async {
    ...
    try {
      thumbnailFile = await VideoCompress.getFileThumbnail(videoPath, quality: AppRes.imageQuality);
    } catch (e) {
  ...
  Future<Duration> getFileDuration(String mediaPath) async {
    ...
    Duration _duration = Duration(seconds: mediaInfoSession.duration?.toInt() ?? 0);
    return _duration;
  }
  ...
  Future<MediaInfo> getVideoInfo(String mediaPath) async {
    try {
      ...
      Loggers.info('File Size : ${getFileSizeString(bytes: mediaInfoSession.filesize ?? 0, decimals: 0)} Mb '
  ...
  Future<XFile?> compressImage(String filePath, String localPath) async {
    try {
      XFile? file = await FlutterImageCompress.compressAndGetFile(filePath, localPath);
  ...
  Future<XFile?> compressProfileImage(String filePath) async {
    ...
    String targetPath = '$localPath/compressed_profile_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
    ...
    do {
      ...
      if (result.lengthInBytes <= maxSizeInBytes) {
        final File _file = File(targetPath)..writeAsBytesSync(result, flush: true);
        return XFile(_file.path);
      }
      quality -= 5;
    } while (quality > 10); // Don't go below 10 quality to avoid super poor images
    File _file = File(targetPath)..writeAsBytesSync(result, flush: true); // Might still be >100KB if image too big
    return XFile(_file.path);
  }
  ...
  Future<bool> _requestImagePermission(ImageSource source) async {
    ...
    await Get.bottomSheet(
        ConfirmationSheet(
          title: isCamera ? LKey.enableCameraAndMicrophoneAccessTitle.tr : LKey.enablePhotoAccessTitle.tr,
          description: isCamera
              ? LKey.enableCameraAndMicrophoneAccessDescription.tr
              : LKey.enablePhotoAccessDescription.trParams({'app_name': AppRes.appName}),
          onTap: () {

...
class BranchIoManager {
  ...
  Future<CustomBranchResponse?> init(
    ...
    final BranchShareData shareData = _generateShareData(type, user: user, post: post, livestream: livestream);
  ...
  Future<CustomBranchResponse?> generateLink(
      {required ShareBranchType type, User? user, Post? post, Livestream? livestream}) async {
    return await init(type: type, user: user, post: post, livestream: livestream);
  }
  ...
  Future<void> shareContent(
      ...
      Livestream? livestream,
      VoidCallback? onShareSuccess}) async {
    CustomBranchResponse? branchResponse =
        await init(type: type, user: user, post: post, livestream: livestream, onShareSuccess: onShareSuccess);
    ...
    if (type == ShareBranchType.qr) {
      _showNativeShareSheet(branchResponse.shareData.title, branchResponse.link);
  ...
  BranchShareData _generateShareData(ShareBranchType type, {User? user, Post? post, Livestream? livestream}) {
    switch (type) {
      ...
      case ShareBranchType.qr:
        ...
        return BranchShareData(
          title: getUserTitle(fullname: user?.fullname ?? '', username: user?.username ?? ''),
      ...
      case ShareBranchType.liveStream:
        ...
        return BranchShareData(
          title: '${livestream?.hostUser?.fullname ?? ''} is Live on ${AppRes.appName}',
  ...
  void _showCustomShareSheet({
    ...
    Get.bottomSheet(
      ShareSheetWidget(
        ...
        isDownloadShow: type == ShareBranchType.post && post?.postType == PostType.reel,
        type: type,

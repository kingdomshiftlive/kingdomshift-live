...
class _AddToCartAnimationState extends State<AddToCartAnimation> {
            ...
            children: animationModels.map<Widget>((model) {
              if (model.showAnimation) {
                return AnimatedPositioned(
                  top: model.animationActive ? model.imageDestPoint.dx + 9 : model.imageSourcePoint.dx,
                  left: model.animationActive ? model.imageDestPoint.dy + 5 : model.imageSourcePoint.dy,
                  ...
                  child: model.rotation
                      ...
                      : AnimatedOpacity(
                          opacity: model.opacity, duration: const Duration(milliseconds: 500), child: model.container),
                );
              } else {
  ...
  Future<void> runAddToCartAnimation(GlobalKey widgetKey) async {
    ...
    int topMargin = 40;
    int leftMargin = 25;
    animationModel.imageSourcePoint = Offset(widgetBounds.top - topMargin, widgetBounds.left + leftMargin);

    animationModel.container = SizedBox(child: (widgetKey.currentWidget! as Container).child);
    ...
    animationModel
      ..imageDestPoint = Offset(widgetBounds.top - 80, widgetBounds.left + leftMargin)

        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.vertical(
                top: Radius.circular(roundedTop ? 8 : 0), bottom: Radius.circular(roundedBottom ? 8 : 0)),
          ),
    ...
    return StoryItem(
        ClipSmoothRect(
          radius: SmoothBorderRadius.vertical(
              top: SmoothRadius(cornerRadius: roundedTop ? 8 : 0, cornerSmoothing: 1),
              bottom: SmoothRadius(cornerRadius: roundedBottom ? 8 : 0, cornerSmoothing: 1)),
          key: key,
          child: Container(
...
class StoryViewState extends State<StoryView> with TickerProviderStateMixin {
  ...
  Widget get _currentView {
    ...
    return SafeArea(
      child: ClipSmoothRect(
        radius: const SmoothBorderRadius.all(SmoothRadius(cornerRadius: 10, cornerSmoothing: 1)),
  ...
  Widget build(BuildContext context) {
    ...
    final bool isMyStory = story?.story?.user == null || story?.story?.user?.id == SessionManager.instance.getUserID();
                ...
                children: <Widget>[
                  ...
                  SafeArea(
                    child: Padding(
                      ...
                      child: AddToCartIcon(
                        cartKey: cartKey,
                        icon: Container(width: 20, height: 20, color: Colors.transparent),
                        badgeOptions: const BadgeOptions(
                            active: true, backgroundColor: Colors.transparent, foregroundColor: Colors.transparent),
                      ),
                    ),
                  ),
                  Visibility(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            return InkWell(
                              onTap: () async {
                                ...
                                if (story != null) {
                                  if (isMyStory) return;
                                  sendReply(textReply: AppRes.storyQuickReplyEmojis[index], item: story);
                                }
                              },
  ...
  void onGiftTap(StoryItem? story) {
    ...
    GiftManager.openGiftSheet(
        ...
        onCompletion: (giftManager) {
          widget.controller.play();
          sendReply(textReply: '', item: story, imageReply: jsonEncode(giftManager.gift.toJson()));
        }).then((value) {
      widget.controller.play();
    });
  }

  void sendReply({required StoryItem item, required String textReply, String? imageReply}) {
    ...
    ChatThread conversation = ChatThread(
        ...
        chatType: user?.isFollowing ?? false ? ChatType.approved : ChatType.request,
        conversationId: [SessionManager.instance.getUserID(), user?.id].conversationId,
        userId: user?.id);
    conversation.chatUser = user?.appUser;

    var chattingController = Get.put(ChatScreenController(conversation.obs), tag: '${conversation.conversationId}');
    if (item.story != null) {
      HapticFeedback.mediumImpact();
      chattingController.sendStoryReply(story: item.story!, textReply: textReply, imageReply: imageReply);
...
class StoryBottomVIew extends StatelessWidget {
  ...
  const StoryBottomVIew(
      {super.key, required this.textEditingController, required this.onSendTap, required this.onGiftTap});
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintStyle: TextStyleCustom.outFitLight300(color: Colors.white, fontSize: 17, opacity: .42),
                      ...
                      suffixIcon: InkWell(
                        ...
                        child: Padding(
                          ...
                          child: Text(
                            LKey.send.tr,
                            style: TextStyleCustom.unboundedMedium500(fontSize: 15, color: Colors.white),
                          ),
                        ),
                      ),
                    ),
                    style: TextStyleCustom.outFitRegular400(color: Colors.white, fontSize: 17),
                  ),
                ),
              ),
              InkWell(
                ...
                child: Container(
                  ...
                  alignment: Alignment.center,
                  child: Image.asset(AssetRes.icGift, height: 20, width: 20, color: whitePure(context)),
                ),
              )
            ],
          ),

...
class ApiService {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (decodedResponse['message'] == 'this user is freezed!') {
          DebounceAction.shared.call(() {
            Get.offAll(() => const SessionExpiredScreen(type: SessionType.freeze));
          });
    ...
    for (final keyName in filesMap.keys) {
      ...
      for (final xFile in files) {
        if (xFile != null && xFile.path.isNotEmpty) {
          ...
          final multipartFile =
              http.MultipartFile(keyName, file.readAsBytes().asStream(), file.lengthSync(), filename: finalFile.name);
  ...
  Future<XFile> _processVideoFile(XFile originalFile) async {
    try {
      ...
      Loggers.info('🎬 Video file detected in API upload, applying compression...');

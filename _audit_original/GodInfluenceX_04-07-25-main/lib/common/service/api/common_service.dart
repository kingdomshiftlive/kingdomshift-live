...
class CommonService {
  ...
  Future<bool> fetchGlobalSettings() async {
    SettingModel settingsModel = await ApiService.instance
        .call(url: WebService.setting.fetchSettings, fromJson: SettingModel.fromJson, cancelAuthToken: true);

    var setting = settingsModel.data;
    logger("setting......    ${jsonEncode(setting?.toJson())}");
    if (setting != null) {
  ...
  Future<FilePathModel> uploadFileGivePath(XFile files, {Function(double percentage)? onProgress}) async {
  ...
  Future<StatusModel> deleteFile(String filePath) async {
    StatusModel model = await ApiService.instance
        .call(url: WebService.setting.deleteFile, param: {Params.filePath: filePath}, fromJson: StatusModel.fromJson);
    return model;
  }
  ...
  Future<List<Places>> searchPlace({String title = ''}) async {
    ...
    Map<String, String> header = {
      Params.authorization: 'Bearer ${settings?.placeApiAccessToken ?? 'PLACE API ACCESS TOKEN EMPTY'}'
    };

    Map<String, dynamic> body = {Params.textQuery: title, Params.maxResultCount: '${AppRes.paginationLimit}'};
    ...
    LocationPlaceModel model = LocationPlaceModel.fromJson(jsonDecode(response.body));
  ...
  Future<List<Places>> searchNearBy({required double lat, required double lon}) async {
    Setting? settings = SessionManager.instance.getSettings();
  ...
  Future<PlaceDetail> getIPPlaceDetail() async {
    Map<String, dynamic> detail = await ApiService.instance.callGet(url: WebService.common.ipApi);

...
class NotificationService {
  ...
  Future<List<AdminNotificationData>> fetchAdminNotifications({int? lastItemId}) async {
  ...
  Future<List<ActivityNotification>> fetchActivityNotifications({int? lastItemId}) async {
  ...
  Future pushNotification(
    ...
    Map<String, dynamic> messageData = {
      ...
      "data": {"title": title, "body": body, 'type': type.type, if (data != null) "notification_data": jsonEncode(data)}
    };
    if (!isIOS) {
    ...
    try {
      http.Response response = await http.post(Uri.parse(WebService.notification.pushNotificationToSingleUser),
          headers: {
            Params.apikey: apiKey,
            Params.authToken: authorizationToken ?? SessionManager.instance.getAuthToken()
          },

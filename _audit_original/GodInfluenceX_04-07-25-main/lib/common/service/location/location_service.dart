...
class LocationService {
  ...
  Future<Position> getCurrentLocation({bool isPermissionDialogShow = false}) async {
    bool serviceEnabled;
  ...
  Future<Position> getCurrentPosition() async {
    ...
    bool hasLocationChanged =
        (latitude - userLatitude).abs() > locationPrecision || (longitude - userLongitude).abs() > locationPrecision;

    if (hasLocationChanged) {
      Future.wait([UserService.instance.updateUserDetails(lat: latitude, lon: longitude)]);
    }
    return position;
  }

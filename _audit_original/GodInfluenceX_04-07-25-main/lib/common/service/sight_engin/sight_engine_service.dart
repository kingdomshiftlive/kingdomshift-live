...
class SightEngineService {
  ...
  Future<void> checkImagesInSightEngine({
    ...
    var request = http.MultipartRequest('POST', Uri.parse('https://api.sightengine.com/1.0/check-workflow.json'));

    request.fields['workflow'] = SessionManager.instance.getSettings()?.sightEngineImageWorkflowId ?? '';
    request.fields['api_user'] = SessionManager.instance.getSettings()?.sightEngineApiUser ?? '';
    request.fields['api_secret'] = SessionManager.instance.getSettings()?.sightEngineApiSecret ?? '';
    ...
    if (sightEngineMediaModel.error != null) {
      ...
      BaseController.share.showSnackBar(sightEngineMediaModel.error?.message ?? '');
      return;
    }
    ...
    if (result == 'accept') {
      ...
      BaseController.share.showSnackBar('${LKey.mediaRejectedAndContainsSuchThings.tr} $summaryDescription');
    }
  }

  Future<void> checkVideoInSightEngine(
      {required XFile xFile, required int duration, required Function() completion}) async {
    if (SessionManager.instance.getSettings()?.isContentModeration == 0) {
      completion();
      return;
    }
    ...
    // Apply video compression before sending to SightEngine
    ...
    request.fields['workflow'] = SessionManager.instance.getSettings()?.sightEngineVideoWorkflowId ?? '';
    request.fields['api_user'] = SessionManager.instance.getSettings()?.sightEngineApiUser ?? '';
    request.fields['api_secret'] = SessionManager.instance.getSettings()?.sightEngineApiSecret ?? '';

    request.files.add(
      http.MultipartFile('media', finalFile.readAsBytes().asStream(), finalFile.lengthSync(),
  ...
  Future<void> chooseTextModeration({required String text, required Function() completion}) async {
    ...
    request.fields.addAll({
      ...
      'mode': 'rules',
      'api_user': SessionManager.instance.getSettings()?.sightEngineApiUser ?? '',
      'api_secret': SessionManager.instance.getSettings()?.sightEngineApiSecret ?? '',
    });
    ...
    if (response.statusCode == 200) {
      ...
      if (textModerationModel.error != null) {
        BaseController.share.stopLoader();
        BaseController.share.showSnackBar(textModerationModel.error?.message ?? '');
        return;
      }
      List<String> words = [];
      ...
      if (words.isEmpty) {
        ...
        BaseController.share.showSnackBar('${LKey.textRejectedAndContainsSuchThings.tr} ${words.join(', ')}');
      }
    } else {
  ...
  /// Process video files for SightEngine by applying compression
  /// This ensures videos sent to SightEngine are optimized
  Future<File> _processVideoForSightEngine(File originalFile) async {
    try {
      ...
      Loggers.info('🎬 Video file detected for SightEngine, applying compression...');

      // Apply video compression
      final compressedFile = await VideoCompressor.compressVideo(originalFile.path);

      if (compressedFile != null) {
        Loggers.success('✅ Video compressed successfully for SightEngine');
        return compressedFile;
      } else {
        Loggers.warning('⚠️ Video compression failed for SightEngine, using original file');
  ...
  bool _isVideoFile(String filePath) {
    ...
    return ['mp4', 'mov', 'avi', 'mkv', 'wmv', 'flv', 'webm', 'm4v'].contains(extension);
  }
}

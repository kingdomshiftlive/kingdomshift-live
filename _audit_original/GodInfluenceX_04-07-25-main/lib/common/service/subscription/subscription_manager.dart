...
class SubscriptionManager with LoggerMixin {
  ...
  bool checkSubscription(CustomerInfo customerInfo) {
    ...
    logs('🔔 Subscription Status: ${isSubscribe.value ? 'Active' : 'InActive'}');
  ...
  Future<LogInResult> login(String appUserID) async {
    logs("🔑 Logging in user: $appUserID");
  ...
  Future<(Offering?, String?)> fetchOfferings() async {
    try {
      ...
      offering.addAll(
          offerings.current?.availablePackages.where((element) => element.packageType == PackageType.custom) ?? []);

      packages.addAll(
          offerings.current?.availablePackages.where((element) => element.packageType != PackageType.custom) ?? []);
      packages.sort((a, b) => b.storeProduct.price.compareTo(a.storeProduct.price));

      logs("✅ Found ${packages.length} packages and ${offering.length} custom offerings");
  ...
  Future<bool?> makePurchase(Package package) async {
    try {
      ...
      logs("${isSubscribed ? '✅' : '❌'} Purchase ${isSubscribed ? 'successful' : 'failed'}");
      return isSubscribed;
    } on PlatformException catch (e) {
  ...
  Future<bool?> restorePurchase() async {
    try {
      ...
      logs("${isSubscribed ? '✅' : '❌'} Purchase restoration ${isSubscribed ? 'successful' : 'failed'}");
      return isSubscribed;
    } on PlatformException catch (e) {

...
class VideoCacheHelper {
  ...
  static Future<FileInfo?> getValidCachedVideo(String url) async {
    ...
    if (timestampStr != null) {
      ...
      if (timestamp != null && DateTime.now().difference(timestamp).inMinutes < expirationMinutes) {
  ...
  static Future<FileInfo> downloadAndCacheVideo(String url) {
    ...
    final future = _cacheManager
        .downloadFile(url, key: storageKey)
        .then((fileInfo) {
  ...
  static Future<void> clearExpiredVideos() async {
    try {
      ...
      if (await dir.exists()) {
        ...
        print('[CLEAR] Total files deleted: ${allFiles.length - validPaths.length}');
      } else {

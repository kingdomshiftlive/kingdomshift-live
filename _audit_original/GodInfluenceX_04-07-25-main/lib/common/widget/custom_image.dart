    ...
    return GestureDetector(
      ...
      child: SizedBox(
        ...
        child: ClipSmoothRect(
          radius: SmoothBorderRadius(cornerRadius: radius, cornerSmoothing: cornerSmoothing),
          child: Stack(
            children: [
              imageUrl.isEmpty
                  ? ImageErrorWidget(
                                ...
                                ? Shimmer.fromColors(
                                    ...
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(
                                          color: bgGrey(context), borderRadius: BorderRadius.circular(radius)),
                                    ))
                                : const SizedBox();
                          },
                ...
                Container(
                  ...
                  decoration: ShapeDecoration(
                    shape: SmoothRectangleBorder(
                      ...
                      side: BorderSide(
                          color: strokeColor ?? whitePure(context).withValues(alpha: .3), width: strokeWidth),
                    ),
                  ),
                ),
            ],
...
class ImageErrorWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: ShapeDecoration(
        shape: SmoothRectangleBorder(
            borderRadius: SmoothBorderRadius(cornerRadius: radius, cornerSmoothing: cornerSmoothing)),
        gradient: isShowPlaceHolder
            ? StyleRes.disabledGreyGradient(opacity: placeHolderColorOpacity ?? 1)
      ...
      child: isShowPlaceHolder
          ? LayoutBuilder(builder: (context, constraints) {
              return Image.asset(placeHolderImage ?? AssetRes.icNoImage,
                  height: constraints.maxHeight / 2, width: constraints.maxWidth / 2, color: textDarkGrey(context));
            })
          : Text(
              (fullName?[0] ?? AppRes.appName[0]).toUpperCase(),
              style: TextStyleCustom.unboundedMedium500(
                  fontSize: size.height / 2, // Fallback to 50 if size is not finite
...
class _MediaDisplayWidgetState extends State<MediaDisplayWidget> {
    ...
    if (_isVideo) {
      _controller = VideoPlayerController.networkUrl(Uri.parse(widget.image!))
        ..initialize().then((_) {
          ...
          setState(() {
            _isInitialized = false; // Fallback to placeholder if initialization fails
          });
        });
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: widget.onTap, // Keep existing onTap for navigating to full-screen player
      child: SizedBox(
            ...
            children: [
              _isInitialized && _controller != null
                  ...
                  : Container(
                      ...
                      decoration: ShapeDecoration(
                        ...
                        gradient: widget.isShowPlaceHolder ? StyleRes.disabledGreyGradient() : StyleRes.themeGradient,
                      ),
              ...
              if (widget.strokeWidth > 0)
                Container(
                  ...
                  decoration: ShapeDecoration(
                    shape: SmoothRectangleBorder(
                      borderRadius: SmoothBorderRadius(cornerRadius: widget.radius),
                      side: BorderSide(
                        color: widget.strokeColor ?? whitePure(context).withValues(alpha: 0.3),

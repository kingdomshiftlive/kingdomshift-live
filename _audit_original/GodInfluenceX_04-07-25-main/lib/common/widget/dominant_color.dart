...
class DominantColors {
  ...
  List<Color> initializeCentroids(List<Color> colors) {
    ...
    for (int i = 1; i < dominantColorsCount; i++) {
      List<double> distances =
          colors.map((color) => centroids.map((centroid) => distance(color, centroid)).reduce(min)).toList();
  ...
  List<Color> extractDominantColors() {
    ...
    while (_isConverging(centroids, oldCentroids)) {
      ...
      List<List<Color>> clusters = List.generate(dominantColorsCount, (index) => []);
  ...
  List<Color> _getPixelsColorsFromHalfImage() {
    ...
    if (image != null) {
      int sampling = 5; //sampling, Adjust as needed. 5 means every 5th pixel, etc.
      ...
      var heightTakenForColors = height / 2; //half of the image is always enough, compared to full image

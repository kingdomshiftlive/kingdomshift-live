...
class SearchResultTile extends StatelessWidget {
  ...
  const SearchResultTile(
      {super.key, required this.onTap, required this.image, required this.title, required this.description});
    ...
    return Column(
      children: [
        InkWell(
          onTap: onTap,
          child: Padding(
            padding: const EdgeInsets.only(left: 12.0, right: 12.0, top: 5, bottom: 9),
            child: Row(
              children: [
                Container(
                  height: 40,
                  width: 40,
                  decoration: BoxDecoration(color: bgMediumGrey(context), shape: BoxShape.circle),
                  alignment: Alignment.center,
                  child: Image.asset(image, height: 20, width: 20, color: textLightGrey(context)),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(title, style: TextStyleCustom.outFitMedium500(fontSize: 16, color: textDarkGrey(context))),
                      Text(description, style: TextStyleCustom.outFitLight300(color: textLightGrey(context))),
                    ],
                  ),
                )
              ],

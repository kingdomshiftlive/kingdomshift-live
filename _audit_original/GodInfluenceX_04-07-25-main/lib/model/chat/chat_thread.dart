...
class ChatThread {
  ...
  AppUser? get chatUser {
    ...
    AppUser? appUser = controller.users.firstWhereOrNull((element) => element.userId == userId);
    if (appUser == null) {
      UserService.instance.fetchUserDetails(userId: userId, onError: () => controller.deleteUser(userId)).then((value) {
        if (value == null) {
...
enum ChatType {
  ...
  static ChatType fromString(String value) {
    return ChatType.values.firstWhereOrNull((e) => e.value == value) ?? ChatType.approved;
  }
}

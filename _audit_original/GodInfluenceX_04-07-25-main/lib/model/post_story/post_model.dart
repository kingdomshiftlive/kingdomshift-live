...
class Post {
  ...
  Post.fromJson(dynamic json) {
    ...
    postType = json['post_type'] != null ? PostType.fromString(json['post_type']) : PostType.none;
    userId = json['user_id'];
  ...
  String get descriptionWithUserName {
    ...
    for (var element in mentionIds) {
      User? user = mentionedUsers?.firstWhereOrNull((u) => u.id == int.parse(element));
      if (user != null) {
        updatedDescription = updatedDescription.replaceAll('@$element', '@${user.username}');
      }
    }
  ...
  void likeToggle(bool isLike) {
    ...
    user?.totalPostLikesCount = ((user?.totalPostLikesCount ?? 0) + i).clamp(0, double.infinity).toInt();
  }
  ...
  String get getThumbnail {
    return (postType == PostType.image ? (images?.first.image ?? '') : (thumbnail ?? ''));
  }

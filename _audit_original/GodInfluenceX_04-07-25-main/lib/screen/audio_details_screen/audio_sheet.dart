...
class _AudioSheetState extends State<AudioSheet> {
  ...
  void onSavedMusic() {
    ...
    final savedMusicIds = user.savedMusicIds?.split(',').where((e) => e.isNotEmpty).map(int.parse).toList() ?? [];
        ...
        Container(
          ...
          child: SafeArea(
            ...
            child: Column(
              children: [
                Container(
                    height: 2, width: 100, color: bgGrey(context), margin: const EdgeInsets.only(top: 10, bottom: 20)),
                Container(
                  height: 115,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      AudioImageWidget(music: widget.music, isPlaying: false.obs),
                      Align(
                          ...
                          child: InkWell(
                            ...
                            child: Obx(
                              () => Image.asset(
                                  music.value?.isSaved ?? false ? AssetRes.icFillBookmark1 : AssetRes.icBookmark,
                ...
                FullNameWithBlueTick(
                  username: widget.music?.user?.username ?? widget.music?.artist ?? '',
                  fontSize: 14,
                ...
                Text(
                  ...
                  style: TextStyleCustom.unboundedSemiBold600(color: textDarkGrey(context), fontSize: 16),
                ),
                Text(
                  LKey.reels.tr,
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: textDarkGrey(context).withValues(alpha: .2), blurRadius: 25)],
          ),
        ),
        CustomImage(
            image: music?.image?.addBaseURL(), size: Size(size.width, size.height), fullName: music?.user?.fullname),
        ClipRRect(
        ...
        if (isPlayIconVisible ?? false)
          Obx(
            () => InkWell(
                ...
                child: Image.asset(!isPlaying.value ? AssetRes.icPlay : AssetRes.icPause,
                    height: 30, width: 30, color: whitePure(context))),
          )
      ],
    );
  }

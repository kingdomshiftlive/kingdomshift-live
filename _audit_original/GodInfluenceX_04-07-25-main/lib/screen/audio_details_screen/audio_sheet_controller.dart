...
class AudioDetailsScreenController extends BaseController {
  ...
  Future<void> fetchReelPostsByMusic() async {
    ...
    List<Post> items = await PostService.instance
        .fetchReelPostsByMusic(musicId: musicId, lastItemId: reelPosts.isEmpty ? null : reelPosts.last.id?.toInt());
  ...
  void onMakeReel() async {
    if (downloadMusicUrl.isEmpty) {
      showLoader();
      downloadMusicUrl = (await DefaultCacheManager().getSingleFile(music.value?.sound?.addBaseURL() ?? '')).path;
      stopLoader();
    }
    audioPlayerController.pausePlayer();
    SelectedMusic selectedMusic = SelectedMusic(Music.fromJson(music.value?.toJson()), 0, downloadMusicUrl, 0);
  ...
  void onPlayPauseMusic() {
    ...
    DefaultCacheManager().getSingleFile(music.value?.sound?.addBaseURL() ?? '').then((value) async {

...
class CreateAudioLiveStreamController extends BaseController {
  ...
  Future<void> createLiveStream() async {
    ...
    if (appIdStr.isEmpty || appSign.isEmpty) {
      ...
      showSnackBar('Live stream configuration is missing. Please contact support.');
      return;
    }
    ...
    try {
      ...
      showSnackBar('Invalid live stream configuration. Please contact support.');
      return;
    }
    ...
    try {
      ...
      Livestream livestream = user.livestream(
        ...
        time: time,
        description: titleController.text.trim().isNotEmpty ? titleController.text.trim() : 'Audio Live Stream',
        restrictToJoin: 0,
        hostViewId: -1,
      );
      ...
      Loggers.info('Livestream User State Model: ${livestreamUserState.toJson()}');

        ...
        ConfirmationSheet(
          ...
          onTap: () async {
            if (user?.isFollowing == true) {
              ...
              if (Get.isRegistered<FollowController>(tag: user?.id.toString())) {
                followController = Get.find<FollowController>(tag: user?.id.toString());
                followController.updateUser(user);
              } else {
                followController = Get.put(FollowController(user.obs), tag: user?.id.toString());
              }
              await followController.followUnFollowUser();
            }
            StatusModel response = await UserService.instance.blockUser(userId: user?.id ?? -1);
  ...
  Future<void> unblockUser(User? user, Function() completion) async {
    await Get.bottomSheet(
        ConfirmationSheet(
          ...
          onTap: () async {
            StatusModel response = await UserService.instance.unBlockUser(userId: user?.id ?? -1);
            if (response.status == true) {
              completion.call();

...
class GenerateContentView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return switch (controller.content.value.type) {
      ...
      PostStoryContentType.reel || PostStoryContentType.storyVideo => CameraEditVideoView(content: controller.content),
    };
  }
}
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          //   },
          // ),
          if ([PostStoryContentType.storyImage, PostStoryContentType.storyText].contains(type))
            Obx(
              () => CustomBorderRoundIcon(
                onTap: controller.changeStoryTime,
                widget: Center(
                  child: Text('${AppRes.storyDurations[controller.currentStoryDurationIndex.value]}s',
                      style: TextStyleCustom.unboundedMedium500(color: whitePure(context)),
          ...
          if (![PostStoryContentType.storyText].contains(content.type))
            CustomBorderRoundIcon(image: AssetRes.icFilter, onTap: controller.onFilterToggle),
          if ([PostStoryContentType.storyText, PostStoryContentType.storyImage].contains(type))
            Obx(() {
              ...
              return CustomBorderRoundIcon(
                ...
                widget: Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      gradient: isTextStory ? gradient : controller.content.value.bgGradient,
                      border: Border.all(color: whitePure(context), width: 2)),
                ),
              );
            }),
          CustomBorderRoundIcon(image: AssetRes.icMusic, onTap: controller.handleMusicSelection),
        ],
      ),
    );
  }
            ...
            AnimatedOpacity(
              ...
              child: IgnorePointer(
                ...
                child: Padding(
                  ...
                  child: ColorFiltersView(onPageChanged: controller.changedFilter, image: content.thumbNail),
                ),
              ),
            ),
            if (music != null)
              ...
              child: Container(
                ...
                child: SizedBox.expand(
                  child: FittedBox(
                    fit: fitType,
                    child: SizedBox(
                        width: videoSize.width, height: videoSize.height, child: VideoPlayer(videoPlayerController!)),
                  ),
                ),
              ),
            ),
            ValueListenableBuilder(
              ...
              builder: (context, value, child) => AnimatedOpacity(
                ...
                child: Container(
                    height: 60,
                    width: 60,
                    decoration: BoxDecoration(color: blackPure(context).withValues(alpha: 0.5), shape: BoxShape.circle),
                    alignment: Alignment.center,
                    child: Image.asset(AssetRes.icPause, width: 35, height: 35, color: bgGrey(context))),
              ),
            )
          ],
        ),

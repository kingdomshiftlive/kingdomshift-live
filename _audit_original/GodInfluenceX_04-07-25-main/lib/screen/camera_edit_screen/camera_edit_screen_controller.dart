...
class CameraEditScreenController extends BaseController {
  ...
  Future<void> addStory(
    ...
    try {
      StoryModel? response = await PostService.instance.createStory(files: {
        ...
        if (type == PostStoryContentType.storyVideo) Params.thumbnail: [XFile(thumbnail!)]
      }, param: {
  ...
  void addStoryResponse(Story story) {
    ...
    Get.isRegistered<ProfileScreenController>(tag: ProfileScreenController.tag)
        ? Get.find<ProfileScreenController>(tag: ProfileScreenController.tag).onAddStory(story)
        : null;

    Get.isRegistered<FeedScreenController>() ? Get.find<FeedScreenController>().onAddStory(story) : null;
  ...
  void onDiscard() {
    Get.bottomSheet(
        ConfirmationSheet(title: LKey.discardEditsTitle.tr, description: LKey.discardEditsMessage.tr, onTap: Get.back));
  }
  ...
  void _initVideoController() async {
    if ([PostStoryContentType.storyImage, PostStoryContentType.storyText].contains(content.value.type)) {
      SelectedMusic? sound = content.value.sound;
      if (sound != null && sound.downloadedURL != null) {
        String audioPath = sound.downloadedURL ?? '';
        await _prepareAudioPlayer(audioPath: audioPath, milliSecond: sound.audioStartMS);
        _playAudioOnly();
      }
      return;
    }

    videoPlayerController.value = VideoPlayerController.file(File(content.value.content ?? ''));

    await videoPlayerController.value?.initialize();

    hasAudio = await RetrytechPlugin.shared.hasAudio(inputPath: content.value.content ?? '') ?? true;
    videoPlayerController.refresh();
    videoPlayerController.value?.setLooping(true);
    content.update((val) => val?.duration = videoPlayerController.value?.value.duration.inSeconds ?? 0);
  ...
  void _setVideoController(VideoPlayerController controller) {
    controller.removeListener(_handleVideoCompletion); // Remove if already exists
  ...
  void _handleVideoCompletion() {
    ...
    final isVideoComplete = (duration - position).inMilliseconds.abs() < 500; // Allow small margin (e.g., 500ms)
  ...
  Future<void> _applyFilterAndAudioToReel() async {
    ...
    String finalPath = inputPath ?? '';
    final List<double> filtersValue = listEquals(selectedFilter.value, defaultFilter) ? [] : selectedFilter.value;
    final mixOriginalAudio = videoPlayerController.value?.value.volume != 0.0;
    final audioStartTimeInMS = double.tryParse('${post.sound?.audioStartMS ?? 0}') ?? 0.0;
  ...
  Future<void> _goToCreateFeedScreen(String videoFilePath) async {
    try {
      ...
      final Uint8List? thumbnailBytes = await MediaPickerHelper.shared.extractThumbnailByte(videoPath: videoFilePath);

      final XFile thumbnailFile = await MediaPickerHelper.shared.extractThumbnail(videoPath: videoFilePath);
  ...
  Future<void> handleStoryUpload() async {
    ...
    final isTextOrImage = [PostStoryContentType.storyImage, PostStoryContentType.storyText].contains(story.type);
    ...
    await SightEngineService.shared.checkVideoInSightEngine(
      ...
      completion: () async {
        ...
        List<double> filtersValue =
            listEquals(selectedFilter.value, filters.first.colorFilter) ? [] : selectedFilter.value;
        String? audioPath = story.sound?.downloadedURL;
        double audioStartMS = double.tryParse('${story.sound?.audioStartMS ?? 0}') ?? 0.0;
  ...
  Future<void> _processImageOrTextStory(int storyDuration) async {
    ...
    showLoader();
    final screenshot = await ScreenshotManager.captureScreenshot(controller.previewContainer);
    if (screenshot == null) {
      stopLoader();
    ...
    MediaPickerHelper.shared.compressImage(screenshot.path, '${localPath}compress_images.jpg').then((value) async {
      stopLoader();
      if (value == null) {
      ...
      await SightEngineService.shared.checkImagesInSightEngine(
        ...
        completion: () async {
          ...
          final audioStartMS = double.tryParse('${story.sound?.audioStartMS ?? 0.0}') ?? 0.0;
          ...
          if (audioPath != null) {
            ...
            updateUploadingProgress(progress: 90);
            await addStory(
                duration: storyDuration, content: imagePath, type: PostStoryContentType.storyImage, musicId: -1);
          }
        },
      );
    });
  ...
  Future<void> handleMusicSelection({SelectedMusic? initialMusic}) async {
    ...
    final duration = isTextOrImage ? selectStorySecond : content.value.duration ?? 0;

    videoPlayerController.value?.pause();

    final SelectedMusic? selectedMusic = await Get.bottomSheet<SelectedMusic?>(
        initialMusic != null
            ? SelectedMusicSheet(selectedMusic: initialMusic, totalVideoSecond: duration)
  ...
  Future<void> _processSelectedMusic(SelectedMusic? selectedMusic, bool isTextOrImage) async {
  ...
  Future<void> _prepareAudioPlayer({required String audioPath, int? milliSecond}) async {
  ...
  changeBg(bool isTextStory) async {
    if (isTextStory) {
      selectedBgIndex.value = (selectedBgIndex.value + 1) % storyGradientColor.length;
    } else {
  ...
  changeStoryTime() async {
    currentStoryDurationIndex.value = (currentStoryDurationIndex.value + 1) % AppRes.storyDurations.length;

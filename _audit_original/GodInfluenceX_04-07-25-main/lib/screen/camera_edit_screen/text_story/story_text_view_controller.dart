...
class StoryTextViewController extends BaseController {
  ...
  void _addFontFamilyList() {
    ...
    outerFontFamilyList
        .assignAll(googleFontsMap.entries.take(10).map((font) => GoogleFontFamily(fontName: font.key)).toList());
  ...
  void onChangeBg() {
    selectedIndex.value = (selectedIndex.value + 1) % GenerateColor.instance.gradientList.length;
  }
  ...
  onSearchFontFamily(String value) {
    if (value.isEmpty) {
      ...
      filteredFontFamilyList.assignAll(
          fontFamilyList.where((font) => font.fontName.toLowerCase().contains((value).toLowerCase())).toList());
    }
  }

  void openFontSheet() async {
    Get.bottomSheet(const GoogleFontFamilySheet(), isScrollControlled: true, ignoreSafeArea: false).then((value) {
      onSearchFontFamily('');
    });
  }

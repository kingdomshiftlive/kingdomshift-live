      ...
      child: Column(
        children: [
          SafeArea(
            ...
            child: ConfirmButton(controller: controller, data: data, textController: textController),
          ),
          Expanded(child: CustomStoryTextField(controller: controller, textController: textController)),
...
class ConfirmButton extends StatelessWidget {
  ...
  const ConfirmButton({super.key, required this.controller, required this.textController, required this.data});
      ...
      child: FittedBox(
        child: InkWell(
          onTap: () {
            ...
            Get.back(result: controller.createUpdatedData(data, textController.text.trim()));
          },
          child: Container(
            height: 30,
            ...
            child: Text(
              LKey.done.tr,
              style: TextStyleCustom.outFitRegular400(fontSize: 17, color: whitePure(context)),
            ),
          ),
        ),
      ),
...
class CustomStoryTextField extends StatelessWidget {
  ...
  const CustomStoryTextField({super.key, required this.controller, required this.textController});
  ...
  TextStyle _getTextStyle(
      {GoogleFontFamily? font, required double fontSize, required Color color, required double opacity}) {
    return font?.style.copyWith(
          fontSize: fontSize,
          color: color.withValues(alpha: opacity),
        ) ??
        TextStyleCustom.outFitMedium500(fontSize: fontSize, color: color, opacity: opacity);
  }
}
        ...
        Obx(() {
          switch (controller.selectorEditorIndex.value) {
            ...
            case StoryTextEditor.opacity:
              return StoryTextFontOpacity(progressValue: controller.selectedTextOpacity, min: 0.0, max: 1.0);
          ...
          child: SafeArea(
            ...
            child: Row(
              ...
              children: controller.editorList.map((editor) {
                return Obx(() {
                  final isSelected = controller.selectorEditorIndex.value == editor;
    ...
    return InkWell(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(text, style: TextStyleCustom.outFitRegular400(fontSize: 12, color: color)),
        ],
      ),
    );
  }

...
class CameraScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(CameraScreenController(cameraType, selectedMusic.obs));
    ...
    return AspectRatio(
      ...
      child: ClipSmoothRect(
        ...
        child: controller.isDeepAr
            ? Obx(
                () {
                  DeepArControllerPlus deepArControllerPlus = controller.deepArControllerPlus.value;
                  return controller.isDeepARInitialized.value
                      ? Transform.scale(
                          scale: deepArControllerPlus.aspectRatio * 0.62, //change value as needed
  ...
  Widget _buildCameraUI(BuildContext context, CameraScreenController controller) {
    return SafeArea(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          CameraTopView(cameraType: cameraType),
          if (cameraType == CameraScreenType.story) _buildTextStoryButton(controller),

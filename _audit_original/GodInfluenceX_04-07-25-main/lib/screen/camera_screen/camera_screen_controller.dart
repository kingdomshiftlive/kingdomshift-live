...
class CameraScreenController extends BaseController with GetSingleTickerProviderStateMixin {
  // Constants
  ...
  void onInit() {
    super.onInit();
    _initialize();
    selectedEffect = Rx(DeepARFilters(id: -1, title: 'None', image: AssetRes.icNoFilter));
  }
  ...
  Future<void> _initializeAudioIfNeeded() async {
    ...
    try {
      await audioPlayer.preparePlayer(path: selectedMusic.value?.downloadedURL ?? '');
      ...
      if (newSecondList.isNotEmpty) {
        ...
        showSnackBar(LKey.recordUpToSeconds.trParams({'second': '$audioSecond'}));
      ...
      if (isStartingRecording.value) {
        await audioPlayer.seekTo(startAudioMs + (progress.value * 1000).toInt());
      } else {
  ...
  void showPermissionDeniedSheet() {
    Get.bottomSheet(
      ConfirmationSheet(
        ...
        description: LKey.cameraMicrophonePermissionDescription.trParams({'app_name': AppRes.appName}),
    ...
    try {
      switch (cameraType) {
        case CameraScreenType.post:
          final mediaFile = await MediaPickerHelper.shared.pickVideo(source: ImageSource.gallery);
        ...
        case CameraScreenType.story:
          ...
          if (mediaFile != null) {
            await (mediaFile.type == MediaType.image ? handleImageStory(mediaFile) : handleVideoStory(mediaFile));
          }
          break;
      }
    } catch (e) {
  ...
  Future<void> handleImageStory(MediaFile file) async {
    ...
    try {
      ...
      await _navigateToEditScreen(PostStoryContentType.storyImage, imagePath, imagePath, bgColor);
    } catch (e) {
  ...
  Future<void> handleVideoStory(MediaFile file) async {
    ...
    await _navigateToEditScreen(PostStoryContentType.storyVideo, videoPath, thumbnailPath, bgColor);
  }
  ...
  Future<void> onVideoRecordingStop() async {
    ...
    try {
      ...
      final XFile thumbnailPath = await MediaPickerHelper.shared.extractThumbnail(videoPath: file.path);
      MediaFile mediaFile = MediaFile(file: file, type: MediaType.video, thumbNail: thumbnailPath);
    ...
    _progressTimer = Timer.periodic(
      ...
      (timer) {
        if (progress.value < selectedSecond.value) {
          ...
          progress.value = (progress.value + increment).clamp(0.0, selectedSecond.value.toDouble());
        } else {
          timer.cancel();
  ...
  Future<void> capturePhoto() async {
    ...
    try {
      ...
      await handleImageStory(MediaFile(file: file, type: MediaType.image, thumbNail: file));
    } catch (e) {
  ...
  Future<void> onMusicTap() async {
    final music = await Get.bottomSheet<SelectedMusic>(MusicSheet(videoDurationInSecond: selectedSecond.value),
  ...
  void onSelectedMusicTap(SelectedMusic? music) async {
    if (music != null && !isStartingRecording.value) {
      final newMusic = await Get.bottomSheet<SelectedMusic>(
        SelectedMusicSheet(selectedMusic: music, totalVideoSecond: selectedSecond.value),
  ...
  Future<void> applyARFilterEffect(DeepARFilters effect) async {
    ...
    try {
      if (effect.id != -1) {
        ...
        final fileInfo = await DefaultCacheManager().downloadFile(effect.filterFile?.addBaseURL() ?? '');

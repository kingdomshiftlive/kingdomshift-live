...
class CameraBottomView extends StatelessWidget {
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              // Gallery button
              CustomBorderRoundIcon(image: AssetRes.icImage, onTap: controller.onMediaTap),
  ...
  Widget _buildStopRecordingButton(CameraScreenController controller) {
    return Obx(() {
      final showStopButton = !controller.isRecording.value && controller.isStartingRecording.value;
      return Visibility(
...
class _EffectList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    List<DeepARFilters> deepARFilters = controller.appSetting?.deepARFilters ?? [];
    deepARFilters.insert(0, DeepARFilters(id: -1, title: 'None', image: AssetRes.icNoFilter));
    return SizedBox(
      height: 89,
        ...
        itemBuilder: (context, index) {
          ...
          return InkWell(
            ...
            child: Obx(() {
              ...
              final borderColor = whitePure(context).withAlpha(isSelected ? 255 : 76);
              return Container(
                    ...
                    Container(
                      ...
                      child: Container(
                        ...
                        child: ClipSmoothRect(
                          ...
                          child: effect.id == -1
                              ? Image.asset(effect.image ?? '', height: 36, width: 36)
                              : Image.network(effect.image?.addBaseURL() ?? '', height: 36, width: 36),
                        ),
                      ),
                    ),
...
class _RecordingDurationSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      final showSelector = !controller.isStartingRecording.value && controller.isSecondListShow.value;
  ...
  Widget _buildDurationItem(BuildContext context, CameraScreenController controller, int index) {
    final second = AppRes.secondList[index];

    return Obx(() {
      final isSelected = second == controller.selectedSecond.value;
      final textStyle = TextStyleCustom.outFitRegular400(color: whitePure(context), fontSize: 15);
          ...
          decoration: isSelected
              ? ShapeDecoration(
                  ...
                  shape: SmoothRectangleBorder(
                    ...
                    side: BorderSide(color: whitePure(context).withAlpha(128), width: 0.5),
                  ))
              : null,
    ...
    return Obx(() {
      return SizedBox(
        ...
        child: GestureDetector(
          ...
          child: CustomPaint(
            painter: DashedCirclePainter(controller.progress / controller.selectedSecond.value),
            child: Center(
              child: controller.isRecording.value ? _buildPauseIndicator(context) : _buildRecordButton(),
            ),
          ),
        ),
      );

...
class ChatScreenController extends BlockUserController with GetTickerProviderStateMixin {
  ...
  void onInit() {
    ...
    documentReceiver = db
        ...
        .collection(FirebaseConst.usersList)
        .doc(myUser?.id.toString());
    chatCollection = db.collection(FirebaseConst.chats).doc(conversationId).collection(FirebaseConst.messages);
  }
  ...
  Future<void> sendMessageToFireStore(
    ...
    if (isSenderUserExist) {
      ...
      if (otherUser != null) {
        ...
        requestType =
            otherUser?.followStatus == 2 || otherUser?.followStatus == 3 ? UserRequestAction.accept.title : null;
      }
  ...
  void pushNotificationToUser(MessageData message) {
    ...
    switch (message.messageType) {
      case MessageType.image:
        bodyMessage = 'Shared a Photo${(message.textMessage ?? '').isNotEmpty ? ': ${message.textMessage}' : ""}';
      case MessageType.video:
        bodyMessage = 'Shared a Video${(message.textMessage ?? '').isNotEmpty ? ': ${message.textMessage}' : ""}';
  ...
  void _pickAudio() async {
    ...
    if (isGranted) {
      audioAnimationController.forward();
      // In audio_waveforms 2.0.0, record() doesn't take encoder parameters
      // Encoder settings are configured via RecorderController constructor or default settings
  ...
  void sendRecordedAudio() async {
    ...
    try {
      ...
      if (recordedFilePath != null) {
        // In audio_waveforms 2.0.0, extract waveform data using preparePlayer
        // The waveform is generated internally when preparing the player
        await playerController.preparePlayer(
          path: recordedFilePath,
          noOfSamples: playerWaveStyle.getSamplesForWidth(wavesWidth),
        );

        // Extract waveform data - in audio_waveforms 2.0.0, we need to use
        // the extractWaveform method or get it from the prepared player state
        // For now, we'll generate a placeholder or use an empty list
        // The actual waveform will be generated by AudioFileWaveforms widget when playing
        List<double> waveData = [];

        // Try to get waveform data if available through player state
        // Note: This is a workaround as extractWaveformData was removed in 2.0.0
        // The waveform will be displayed using AudioFileWaveforms widget when the message is viewed
        try {
          // Generate a simple placeholder waveform
          // This ensures the UI doesn't break, but the actual waveform will be generated on playback
          final samples = playerWaveStyle.getSamplesForWidth(wavesWidth);
          waveData = List.generate(samples, (index) => 0.3); // Placeholder values
        } catch (e) {
          Loggers.error('Error generating waveform placeholder: $e');
          waveData = [];
        }
  ...
  void onPostTap(Post post) async {
    ...
    switch (type) {
      ...
      case PostType.podcast:
        Get.to(() => ReelsScreen(reels: [post].obs, position: 0, isFromChat: true, hasMoreData: false.obs));
        break;
      case PostType.image:
      case PostType.text:
  ...
  void _addUsersFirebaseFireStore() async {
    ...
    DocumentReference otherUserRef = collectionUsersRef.doc(conversationUser.value.userId.toString());

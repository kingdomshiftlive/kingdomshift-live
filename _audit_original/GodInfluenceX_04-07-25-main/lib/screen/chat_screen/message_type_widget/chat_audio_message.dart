...
class ChatAudioMessage extends StatelessWidget {
  ...
  const ChatAudioMessage({super.key, required this.message, required this.controller});

  @override
  Widget build(BuildContext context) {
    List<double> waves = message.waveData?.split(',').map((e) => double.parse(e)).toList() ?? [];
    ...
    return Container(
      ...
      child: Obx(() {
        ...
        bool isPlaying = (playerValue.state == PlayerState.playing) && (playerValue.id == message.id);
        return Row(
          spacing: 5,
          children: [
            InkWell(
              ...
              child: Container(
                  ...
                  child: GradientIcon(
                      child: Image.asset(isPlaying ? AssetRes.icPause : AssetRes.icPlay, width: 25, height: 25))),
            ),
            SizedBox(
              width: wavesWidth,
              height: 50,
                  ...
                  : Row(
                      ...
                      return Expanded(
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: SmoothRectangleBorder(
                                borderRadius: SmoothBorderRadius(cornerRadius: 15, cornerSmoothing: 0)),

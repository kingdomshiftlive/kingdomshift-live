...
class ChatPostMessage extends StatelessWidget {
  ...
  const ChatPostMessage({super.key, required this.message, required this.controller});
...
class ChatPostReelVideoMessage extends StatelessWidget {
  ...
  const ChatPostReelVideoMessage({super.key, required this.post, required this.controller});
      ...
      child: Column(
        ...
        children: [
            ...
            child: InkWell(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Image.asset(AssetRes.icChatPlay, width: 54, height: 54, color: whitePure(context))
                ],
              ),
            ),
          ),
          Visibility(
            ...
            child: ChatPostText(description: post.descriptionWithUserName, topPadding: 5),
          )
        ],
      ),
    );
...
class ChatPostImageMessage extends StatelessWidget {
  ...
  const ChatPostImageMessage({super.key, required this.post, required this.controller});

  @override
  Widget build(BuildContext context) {
    double width = Get.width / 1.4;
    bool isVideo = post.postType == PostType.video || post.postType == PostType.podcast;
    return InkWell(
      onTap: () => controller.onPostTap(post),
      child: Container(
        constraints: BoxConstraints(maxWidth: width),
        ...
        child: Column(
          ...
          children: [
            ChatPostProfile(user: post.user),
            if (!isVideo) PostImageView(post: post, height: width, margin: EdgeInsets.zero, radius: 0),
            if (isVideo) PostVideoView(post: post, margin: EdgeInsets.zero, radius: 0, isFromChat: true),
            Visibility(
              replacement: const Padding(padding: EdgeInsets.all(10.0), child: SizedBox(height: 0)),
              visible: post.description != null,
...
class ChatPostTextMessage extends StatelessWidget {
  ...
  const ChatPostTextMessage({super.key, required this.post, required this.controller});
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ChatPostProfile(user: post.user),
            ChatPostText(description: post.descriptionWithUserName, topPadding: 0),
          ],
        ),
      ),
    );
        ...
        children: [
          ...
          Expanded(
            child: FullNameWithBlueTick(
                username: user?.username ?? '', fontColor: whitePure(context), iconSize: 18, isVerify: user?.isVerify),
          )
        ],
      ),
    );
...
class ChatPostText extends StatelessWidget {
  ...
  const ChatPostText({super.key, required this.description, this.topPadding = 10});

  @override
  Widget build(BuildContext context) {
    TextStyle collapsedStyle = TextStyleCustom.outFitLight300(fontSize: 15, color: whitePure(context), opacity: .8);

    return Padding(
      padding: EdgeInsets.only(left: 10.0, right: 10, bottom: 10, top: topPadding),
      child: ReadMoreText(
        description,

...
class ChatStoryReplyMessage extends StatelessWidget {
  ...
  const ChatStoryReplyMessage({super.key, required this.controller, required this.message, required this.isMe});

  @override
  Widget build(BuildContext context) {
    double width = Get.width / 2.3;
    Story story = Story.fromJsonWithUser(jsonDecode(message.storyReplyMessage ?? ''));
        ...
        children: [
                    ...
                    children: [
                      if (!isMe)
                        LayoutBuilder(builder: (context, constraints) {
                          return Container(
                            width: 3,
                            height: isStoryUnAvailable ? 30 : constraints.maxHeight - 15,
                      ...
                      isStoryUnAvailable
                          ...
                          : InkWell(
                              ...
                              child: Container(
                                constraints: BoxConstraints(maxWidth: width - 40, maxHeight: 200),
                                decoration: ShapeDecoration(
                                    shape: SmoothRectangleBorder(
                                ...
                                child: Padding(
                                    ...
                                    child: CustomImage(
                                        size: Size(width, 200),
                                        image: (story.type == 0 ? story.content : story.thumbnail)?.addBaseURL(),
                                        cornerSmoothing: 1,
                                        radius: 12,
                      ...
                      if (isMe)
                        LayoutBuilder(builder: (context, constraints) {
                          return Container(
                            width: 3,
                            height: isStoryUnAvailable ? 30 : constraints.maxHeight - 10,
                            decoration: ShapeDecoration(
                              shape: SmoothRectangleBorder(
                                  borderRadius: SmoothBorderRadius(cornerRadius: 30, cornerSmoothing: 1)),
                              color: disableGrey(context),
                  ...
                  (message.textMessage ?? '').isEmpty
                      ? NotificationGiftIcon(
                          gift: (message.imageMessage ?? '').isEmpty
                              ? null
                              : Gift.fromJson(jsonDecode(message.imageMessage ?? '')),
                        )
          ...
          if (isEmojiText && !isStoryUnAvailable)
            Positioned(
              ...
              left: !isMe ? null : 10,
              right: !isMe ? 10 : null,
              child: Text(message.textMessage ?? '', style: const TextStyle(fontSize: 50, color: Colors.black)),
            )
        ],
      ),
    );

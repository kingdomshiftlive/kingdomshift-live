        ...
        return LoadMoreWidget(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              bool isMe = message.chatUser?.userId == SessionManager.instance.getUserID();
              return Container(
                padding: const EdgeInsets.only(left: 10, right: 10, top: 7, bottom: 7),
                decoration: ShapeDecoration(
                    shape:
                        SmoothRectangleBorder(borderRadius: SmoothBorderRadius(cornerRadius: 15, cornerSmoothing: 1))),
                child: Column(
                  ...
                  children: [
                    ContextMenuWidget(
                      menuProvider: (_) {
                        return Menu(
                          children: [
                            MenuAction(title: LKey.deleteForYou.tr, callback: () => controller.onDeleteForYou(message)),
                            if (isMe) MenuAction(title: LKey.unSend.tr, callback: () => controller.onUnSend(message)),
                          ],
                        );
                      },
                      child: Container(
                        ...
                        child: switch (message.messageType) {
                          MessageType.image => ChatMediaMessage(isMe: isMe, message: message, controller: controller),
                          MessageType.video => ChatMediaMessage(isMe: isMe, message: message, controller: controller),
                          MessageType.post => ChatPostMessage(message: message, controller: controller),
                          MessageType.audio => ChatAudioMessage(message: message, controller: controller),
                          MessageType.text => ChatTextMessage(isMe: isMe, message: message),
                          MessageType.gift => ChatGiftMessage(message: message, isMe: isMe),
                          MessageType.gif => ChatGIFMessage(message: message),
                          MessageType.storyReply =>
                            ChatStoryReplyMessage(controller: controller, message: message, isMe: isMe),
                          null => const SizedBox(),
                        },
                      ),
                    ),

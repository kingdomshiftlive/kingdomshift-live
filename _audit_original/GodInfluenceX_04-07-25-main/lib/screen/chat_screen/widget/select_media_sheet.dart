          ...
          child: SafeArea(
            ...
            child: Column(
              children: [
                ...
                InkWell(
                  onTap: () async {
                    XFile? file = await MediaPickerHelper.shared.pickImage(source: ImageSource.camera);
                    if (file != null) {
                      onSelectMedia.call(MediaFile(file: file, type: MediaType.image, thumbNail: file));
                    } else {
                  ...
                  child: Container(
                    ...
                    child: Text(LKey.image.tr, style: TextStyleCustom.unboundedLight200(fontSize: 17)),
                  ),
                ),
                const CustomDivider(),
                InkWell(
                  onTap: () async {
                    MediaFile? file = await MediaPickerHelper.shared.pickVideo(source: ImageSource.camera);
                    if (file != null) {
                      onSelectMedia.call(MediaFile(file: file.file, type: MediaType.video, thumbNail: file.thumbNail));
                    } else {
                  ...
                  child: Container(
                    ...
                    alignment: Alignment.center,
                    padding: const EdgeInsets.only(bottom: 20, top: 10),
                    child: Text(LKey.video.tr, style: TextStyleCustom.unboundedLight200(fontSize: 17)),
                  ),
                ),
              ],
            ),

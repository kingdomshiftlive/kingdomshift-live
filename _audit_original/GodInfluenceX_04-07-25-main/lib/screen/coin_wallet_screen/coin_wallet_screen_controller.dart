...
class CoinWalletScreenController extends BaseController {
    ...
    for (var data in settings!.coinPackages!) {
      if (data.status == 1) {
        for (var element in items) {
          if ([data.appstoreProductId, data.playStoreProductId].contains(element.storeProduct.identifier)) {
            coinPlans.add(CoinPlan(data.coinAmount ?? 0, data.id ?? -1, element.storeProduct.identifier,
  ...
  void onPurchase(CoinPlan offer) {
    ...
    Package package = offerings.firstWhere((element) => element.storeProduct.identifier == offer.id);
    SubscriptionManager.shared.makePurchaseCustom(package).then((value) async {
      if (value != null) {
        ...
        User? user = await GiftWalletService.instance.buyCoins(id: offer.coinPackageId, purchasedAt: millis.toString());
        stopLoader();
        if (user != null) {
          User? user = await UserService.instance.fetchUserDetails(userId: myUser.value?.id);
          if (user != null) {
            myUser.value = user;

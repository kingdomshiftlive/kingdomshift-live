...
class CommentSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller =
        Get.put(CommentSheetController(post.obs, comment, replyComment, isFromNotification, CommentHelper()));
    return Container(
      margin: isFromBottomSheet ? EdgeInsets.only(top: AppBar().preferredSize.height * 2.5) : null,
        ...
        children: [
          if (isFromBottomSheet)
            Obx(() => BottomSheetTopView(
                title: '${(controller.post.value?.comments ?? 0).toInt().numberFormat} ${LKey.comments.tr}',
                ...
                controller.getCommentsList.isEmpty && controller.isLoading.value
                    ...
                    : controller.getCommentsList.isEmpty && !controller.isLoading.value
                        ? (!isFromBottomSheet
                            ? const SizedBox()
                            : NoDataView(
                                title: LKey.postCommentEmptyTitle.tr, description: LKey.postCommentEmptyDescription.tr))
          ...
          if (isFromBottomSheet)
            CommentBottomTextFieldView(helper: controller.commentHelper, isFromBottomSheet: isFromBottomSheet),
        ],
      ),
    );
  }

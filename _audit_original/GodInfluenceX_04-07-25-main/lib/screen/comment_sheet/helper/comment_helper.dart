...
class CommentHelper {
  ...
  DetectableTextEditingController detectableTextController = DetectableTextEditingController(
      detectedStyle: TextStyleCustom.outFitMedium500(fontSize: 16, color: ColorRes.themeGradient2),
  ...
  void searchMentionUsers(String value) {
    ...
    DebounceAction.shared.call(
      () async {
        List<User> items = await UserService.instance
            .searchUsers(keyWord: value.split('@')[1], limit: AppRes.paginationLimitDetectWord);
        searchUsers
          ..clear()
          ..addAll(items);

        for (var item in items) {
          bool isExist = allMentionUsers.any((element) => element.id == item.id);
          if (!isExist) {
  ...
  void searchHashTag(String value) async {
    ...
    DebounceAction.shared.call(() async {
      List<Hashtag> items = await SearchService.instance.searchHashtags(keyword: value.split('#')[1]);
      hashTags.clear();
  ...
  void appendDetection(dynamic item, DetectType detectType, {required int type}) {
    ...
    if (type == 0) {
      ...
      detectType == DetectType.atSign ? isMentionUserView.value = false : isHashTagView.value = false;
      ...
      if (detection != null) {
        ...
        if (cursorPosition != -1) {
          String textBeforeCursor = detectableTextController.text.substring(0, cursorPosition);
          String textAfterCursor = detectableTextController.text.substring(cursorPosition);
          ...
          if (textBeforeCursor.endsWith(detection)) {
            ...
            detectableTextController.selection = TextSelection.fromPosition(
              TextPosition(offset: textBeforeCursor.length - detection.length + ' $symbol$text '.length),
            );
          }
        }
      }
    ...
    if (detectType == DetectType.atSign && allMentionUsers.every((element) => element.id != item.id)) {
      allMentionUsers.add(item);
    }
  }
  ...
  Future<void> onCommentPost({
    ...
    bool isModerationPassed = false;
    await SightEngineService.shared.chooseTextModeration(
      text: description,
      completion: () {
    ...
    Comment? replyComment =
        this.replyComment.value == null ? null : Comment.fromJson(this.replyComment.value?.toJson());
    ...
    Comment? comment = isReply
        ? await _handleReplyComment(description, mentionUserIds: mentionUserIds.join(','))
        : await handleAddComment(description, commentType, reel, mentionUserIds: mentionUserIds.join(','));
  ...
  List<int> _extractMentionUserIds(String description) {
    Set<String> mentions = TextPatternDetector.extractDetections(description, AppRes.detectableReg)
        .where((e) => e.contains('@'))
        .map((e) => e.replaceAll('@', ''))
        .toSet();
    ...
    for (final username in mentions) {
      final user = allMentionUsers.firstWhereOrNull((u) => u.username == username);
  ...
  String _replaceMentionsWithUserIds(String description, List<User> mentionUsers) {
    final regex = RegExp(r'@([\w.-]+)');

    return description.replaceAllMapped(regex, (match) {
      String? username = match.group(1); // Now correctly captures the username without '@'
  ...
  void _sendCommentNotification(Post reel, Comment comment, bool isReply, Comment? replyComment) {
    if (isReply) {
      final reply = replyComment;
      if (reply != null && reply.userId != myUser?.id && reply.user?.notifyPostComment == 1) {
        FirebaseNotificationManager.instance.sendLocalisationNotification(
          ...
          body: NotificationInfo(id: reel.id, commentId: comment.commentId, replyCommentId: comment.id),
  ...
  Future<void> _notifyMentionedUsers(Post reel, Comment comment, bool isReply) async {
    ...
    for (final user in finalUsers) {
      if (user.notifyMention == 1 && user.id != myUser?.id) {
        batch.add(FirebaseNotificationManager.instance.sendLocalisationNotification(LKey.notifyMentionedInComment,
            deviceToken: user.deviceToken ?? '',
            deviceType: user.device ?? 0,
            languageCode: user.appLanguage,
            body: NotificationInfo(id: reel.id, commentId: commentId, replyCommentId: replyCommentId),
            type: NotificationType.post));
  ...
  Future<Comment?> handleAddComment(String description, CommentType type, Post post, {String? mentionUserIds}) async {
    ...
    Comment? comment = await PostService.instance
        .addComment(postId: postId, comment: description, mentionUserIds: mentionUserIds, type: type.value);
    return comment;
  }

  Future<Comment?> _handleReplyComment(String description, {String? mentionUserIds}) async {
    ...
    Comment? comment = await PostService.instance
        .replyToComment(commentId: commentId, reply: description, mentionUserIds: mentionUserIds);
  ...
  Future<void> fetchUrlPreview() async {
    List<String> urls = TextPatternDetector.extractDetections(detectableTextController.text.trim(), atSignUrlRegExp);

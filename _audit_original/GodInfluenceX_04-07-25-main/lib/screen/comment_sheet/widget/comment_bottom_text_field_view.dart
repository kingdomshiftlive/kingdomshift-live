...
class CommentBottomTextFieldView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = this.controller ?? Get.find<CommentSheetController>();
    return Container(
              ...
              child: Row(
                ...
                children: [
                  Obx(() {
                    ...
                    return CustomImage(
                        size: const Size(46, 46), image: user?.profilePhoto?.addBaseURL(), fullName: user?.fullname);
                  }),
                  Expanded(
                    child: Obx(
                      () {
                        return Container(
                          decoration: ShapeDecoration(
                            shape: SmoothRectangleBorder(
                              borderRadius: SmoothBorderRadius(cornerRadius: helper.isReplyUser.value ? 20 : 30),
                            ),
                          ),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (helper.isReplyUser.value) ReplyingToUserText(helper: helper),
                              Container(
                                ...
                                child: DetectableTextField(
                                  onTap: () async {
                                    if (!isFromBottomSheet) {
                                      ...
                                      Scrollable.ensureVisible(controller.commentKey.currentContext!,
                                  ...
                                  style: TextStyleCustom.outFitRegular400(
                                      color: textColor ?? textDarkGrey(context), fontSize: 16),
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    suffixIcon: Padding(
                                      ...
                                      child: Obx(() {
                                        bool isTextComment = helper.isTextComment.value;
                                        return InkWell(
                                          ...
                                          child: AnimatedContainer(
                                            ...
                                            child: SingleChildScrollView(
                                              ...
                                              child: Padding(
                                                ...
                                                child: Text(isTextComment ? LKey.post.tr : LKey.gif.tr,
                                                    style: TextStyleCustom.unboundedMedium500(
                                                      fontSize: 15,
                                                      color: themeAccentSolid(context),
                                                    ),
                                                    textAlign: isTextComment ? TextAlign.start : TextAlign.end),
                                              ),
                                            ),
                                          ),
                                        );
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text('${LKey.replyingTo.tr} @${helper.replyComment.value?.user?.username}',
                    maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: textLightGrey(context))),
              ),
              InkWell(
                  onTap: helper.onCloseReply,
                  child: Icon(Icons.close_rounded, color: textLightGrey(context), size: 20)),
            ],
          ),
        ),
      ],

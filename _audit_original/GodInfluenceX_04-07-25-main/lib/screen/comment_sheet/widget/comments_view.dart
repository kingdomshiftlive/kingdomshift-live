...
class CommentsView extends StatelessWidget {
  ...
  const CommentsView(
      {super.key, required this.controller, this.shrinkWrap = true, this.cardBackgroundColor, this.cardTextColor});
...
class CommentItems extends StatelessWidget {
  ...
  const CommentItems(
      {super.key, required this.comment, required this.controller, this.backgroundColor, this.textColor});
...
class ReplyCommentSectionWidget extends StatelessWidget {
  final int commentId;
  final Comment comment;
  final CommentSheetController controller; // Replace with actual controller type

  const ReplyCommentSectionWidget(
      {super.key, required this.commentId, required this.comment, required this.controller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      List<Comment> replyComments = controller.getReplyCommentsList[commentId] ?? [];
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ReplyCommentsView(replyComments: replyComments, controller: controller),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 10.0),
                  child: controller.isFetchReplyComment.value && comment.id == controller.replyCommentId.value
                      ...
                      : InkWell(
                          ...
                          child: Row(
                            children: [
                              Container(height: 1, width: 30, color: textLightGrey(context)),

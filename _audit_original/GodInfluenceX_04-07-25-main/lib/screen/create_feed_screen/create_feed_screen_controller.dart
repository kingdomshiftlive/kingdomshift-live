...
class CreateFeedScreenController extends BaseController {
  ...
  void handleUpload() async {
    ...
    } else if (createType == CreateFeedType.podcast) {
      // Ensure video is selected for podcast
      if (video.value == null) {
        showSnackBar('Please select a video for your podcast');
        return;
      }
      Loggers.info('Podcast Video UploadParams: $postParams');
  ...
  void _addTextDetections(Map<String, dynamic> params, String rawDescription) {
    ...
    final processedDescription = _processMentions(rawDescription, mentionUsernames);
  ...
  String _processMentions(String text, List<String> mentionUsernames) {
    ...
    for (final username in mentionUsernames) {
      final user = commentHelper.allMentionUsers.firstWhereOrNull((u) => u.username == username);
  ...
  void runContentModerationAndUpload({required String description, required Map<String, dynamic> params}) {
    // Force video moderation for podcast
    if (createType == CreateFeedType.podcast) {
      Loggers.info('Running SightEngine video moderation for Podcast...');
  ...
  Future<void> _uploadPostHandler(Map<String, dynamic> postParams) async {
    ...
    try {
      ...
      switch (createType) {
        ...
        case CreateFeedType.feed || CreateFeedType.podcast:
          if (createType == CreateFeedType.podcast) {
            Loggers.info('Uploading Podcast...');
          ...
          switch (feedPostType.value) {
            ...
            case FeedPostType.text:
              ...
              if (commentHelper.metaData.value != null) {
                postParams[Params.metadata] = jsonEncode(commentHelper.metaData.value);
              }
              postResponse = await AddPostStoryService.instance.addPostFeedText(param: postParams);
              break;
            case FeedPostType.video:
      ...
      if (postResponse?.status == true) {
        ...
        if (Get.isRegistered<ProfileScreenController>(tag: ProfileScreenController.tag)) {
          final profileController = Get.find<ProfileScreenController>(tag: ProfileScreenController.tag);
  ...
  Future<void> _notifyMentionedUsers(Post post) async {
    ...
    for (final mentionUser in (post.mentionedUsers ?? [])) {
      if (mentionUser.notifyMention == 1 && mentionUser.id != myUser?.id) {
        batch.add(FirebaseNotificationManager.instance.sendLocalisationNotification(
  ...
  Future<PostModel?> _handleReelUpload(PostStoryContent? content, Map<String, dynamic> params) async {
    if (content == null) {
    ...
    if (hasAudio) {
      if (selectedMusic == null) {
        ...
        bool? success = await _retrytechPlugin.extractAudio(inputPath: videoPath, outputPath: extractAudioPath);
        ...
        final XFile? profileImage = await _loadProfileOrThumbnailImage(content.thumbNail);
    ...
    // Step 5: Apply final video compression before upload
    Loggers.info('Applying final video compression...');
    double progress = 10;
    final finalVideoFile = await _applyFinalVideoCompression(videoPath, progressCallback: (double v) {
      progress = (70 * v).clamp(10, 70);
      updateUploadingProgress(progress: progress);
    });
    if (finalVideoFile == null) {
      deleteFiles([videoPath, content.thumbNail, extractAudioPath]);
      return failedResponseSnackBar(message: 'Video compression failed');
    }

    // Step 6: Upload video & thumbnail
    Loggers.info('Uploading video...');
    FilePathModel uploadedVideo = await CommonService.instance.uploadFileGivePath(finalVideoFile);

    Loggers.info('Uploading thumbnail...');
    FilePathModel uploadedThumb = await CommonService.instance.uploadFileGivePath(XFile(content.thumbNail ?? ''));

    // Step 7: Check upload success
    if (uploadedVideo.status == false || uploadedThumb.status == false) {
      deleteFiles([videoPath, content.thumbNail, extractAudioPath, finalVideoFile.path]);
    ...
    try {
      ...
      PostModel result = await AddPostStoryService.instance.addPostReel(param: params);
      return result;
    } catch (e) {
  ...
  Future<XFile?> _loadProfileOrThumbnailImage(String? fallbackThumb) async {
    try {
      ...
      final file = await DefaultCacheManager().getSingleFile(profileImage.addBaseURL());
  ...
  Future<PostModel?> _handleImageUpload(Map<String, dynamic> params) async {
    ...
    List<XFile> filterImages = await Future.wait(
      images.map((image) async {
        ...
        if (isFilterApply) {
          ...
          String outputPath = '$localPath${images.indexOf(image)}filter_image.jpg';
          bool? result = await _retrytechPlugin.applyFilterToImage(
              inputPath: image.media.path, filterValues: image.colorFilter, outputPath: outputPath);
          if (result == true) {
    ...
    if (setting.value?.isCompress == 1) {
      for (int i = 0; i < filterImages.length; i++) {
        ...
        XFile? _compressImageFile =
            await MediaPickerHelper.shared.compressImage(imageFile.path, '${localPath}compress_images_$i.jpg');
    ...
    for (var image in compressImages) {
      await CommonService.instance.uploadFileGivePath(XFile(image)).then((result) {
        if (result.status == true && result.data != null) {
          ...
          deleteFiles(
              images.map((e) => e.media.path).toList() + filterImages.map((e) => e.path).toList() + compressImages);
    ...
    deleteFiles(images.map((e) => e.media.path).toList() + filterImages.map((e) => e.path).toList() + compressImages);

    Loggers.info('Uploading final post... $params');

    // Step 5: Upload final post
    try {
      final postResult = await AddPostStoryService.instance.addPostFeedImage(param: params);
      return postResult;
    } catch (e) {
      Loggers.error('Exception during post upload: $e');
      return failedResponseSnackBar(message: '$e');
  ...
  Future<PostModel?> _handleVideoUpload(Map<String, dynamic> params) async {
    ...
    if (isApplyFilter) {
      ...
      if (result == true) {
        ...
        finalThumbnailFile = await MediaPickerHelper.shared.extractThumbnail(videoPath: finalVideoFile.path);
      } else {
    ...
    if (setting.value?.isCompress == 1) {
      ...
      XFile? compressVideoFile = await MediaPickerHelper.shared.compressVideo(finalVideoFile.path, '');
      ...
      XFile? compressThumbFile =
          await MediaPickerHelper.shared.compressImage(finalThumbnailFile.path, '${localPath}compress_video_thumb.jpg');
    ...
    double progress = 20;
    // Step 4: Apply final video compression before upload
    Loggers.info('Applying final video compression...');
    final finalCompressedVideoFile =
        await _applyFinalVideoCompression(finalVideoFile.path, progressCallback: (double v) {
      progress = (80 * v).clamp(20, 80);
      updateUploadingProgress(progress: progress);
    });
    if (finalCompressedVideoFile == null) {
      deleteFiles([finalVideoFile.path, finalThumbnailFile.path]);
      return failedResponseSnackBar(message: 'Final video compression failed');
    }

    // Step 5: Upload video
    Loggers.info('Uploading video...');
    FilePathModel uploadedVideo = await CommonService.instance.uploadFileGivePath(finalCompressedVideoFile);

    Loggers.info('Uploading thumbnail...');
    FilePathModel uploadedThumbnail = await CommonService.instance.uploadFileGivePath(finalThumbnailFile);
    deleteFiles([finalVideoFile.path, finalThumbnailFile.path, finalCompressedVideoFile.path]);
    ...
    try {
      ...
      if (createType == CreateFeedType.podcast) {
        result = await AddPostStoryService.instance.addPostFeedPodcast(param: params);
      } else {
        result = await AddPostStoryService.instance.addPostFeedVideo(param: params);
      }
  ...
  /// Apply final video compression before upload - this ensures all videos are optimized
  /// regardless of existing compression settings
  Future<XFile?> _applyFinalVideoCompression(String videoPath, {dynamic Function(double)? progressCallback}) async {
    try {
      Loggers.info('🎬 Applying final video compression before upload...');

      final compressedFile = await VideoCompressor.compressVideo(videoPath, progressCallback: progressCallback);

      if (compressedFile != null) {
        Loggers.success('✅ Final video compression completed successfully');
        return XFile(compressedFile.path);
      } else {
        Loggers.warning('⚠️ Final video compression failed, using original file');
        return XFile(videoPath);
      }
    } catch (e) {
      Loggers.error('❌ Error during final video compression: $e');
      return XFile(videoPath); // Return original file as fallback
    }
  }
  ...
  void onMediaTap(FeedPostType type) {
  ...
  Future<void> selectImages() async {
    final int remainingSlots = setting.value?.maxImagesPerPost ?? AppRes.imageLimit - images.length;

    if (remainingSlots >= 2) {
      final List<XFile> imageFiles = await MediaPickerHelper.shared.multipleImages(limit: remainingSlots);

      images.addAll(imageFiles.map(
        (file) => ImageWithFilter(media: file, thumbnail: file),
      ));
    } else {
      final XFile? imageFile = await MediaPickerHelper.shared.pickImage(source: ImageSource.gallery);
  ...
  Future<void> pickVideo() async {
    MediaFile? file = await MediaPickerHelper.shared.pickVideo(source: ImageSource.gallery);
    if (file != null) {
      video.value = ImageWithFilter(media: file.file, thumbnail: file.thumbNail);
      videoPlayerController.value = VideoPlayerController.file(File(file.file.path))
  ...
  void onChangeReelCover() async {
    XFile? file = await MediaPickerHelper.shared.pickImage(source: ImageSource.gallery);
    if (file != null) {
...
class ImageWithFilter {
  ...
  ImageWithFilter({required this.media, this.colorFilter = defaultFilter, required this.thumbnail});
}

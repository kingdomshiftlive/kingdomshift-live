          ...
          Expanded(
            child: DetectableTextField(
              onTap: () async {
                ...
                Scrollable.ensureVisible(globalKey.currentContext!, duration: const Duration(milliseconds: 350));
              },
              ...
              decoration: InputDecoration(
                  ...
                  hintStyle: TextStyleCustom.outFitLight300(color: textLightGrey(context), fontSize: 16)),
                ...
                (index) {
                  ...
                  return IconWithText(
                    ...
                    onTap: () {
                      ...
                      switch (index) {
                        case 0:
                          Get.bottomSheet(const MentionSheet(), isScrollControlled: true).then((value) {
                            if (helper.detectableTextController.typingDetection == null) {
                              return;
                            }
                            helper.onChanged(helper.detectableTextController.typingDetection?.split('@')[1] ?? '');
                          });
                        case 1:
                          Get.bottomSheet(const HashTagSheet(), isScrollControlled: true).then((value) {
                            if (helper.detectableTextController.typingDetection == null) {
                              return;
                            }
                            helper.onChanged(helper.detectableTextController.typingDetection?.split('#')[1] ?? '');
                        ...
                        case 2:
                          Get.bottomSheet(LocationSheet(onLocationTap: controller.onLocationTap),
...
class IconWithText extends StatelessWidget {
  ...
  const IconWithText({super.key, required this.icon, required this.text, this.onTap});
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Image.asset(icon, color: textDarkGrey(context), height: 19, width: 19),
              const SizedBox(width: 5),
              Text(text, style: TextStyleCustom.outFitLight300(color: textDarkGrey(context)))
            ],
          ),
        ),
      ),

...
class _LocationSheetState extends State<LocationSheet> {
          ...
          CustomSearchTextField(
            ...
            onChanged: (value) {
              ...
              DebounceAction.shared.call(() async {
                places.value = [];
                List<Places> _place = await CommonService.instance.searchPlace(title: value);
  ...
  Future<void> _fetchInitLocation() async {
    ...
    isLocationLoading.value = true;
    try {
      position = await LocationService.instance.getCurrentLocation(isPermissionDialogShow: true);
    } catch (e) {
  ...
  Future<void> searchNearBy(double lat, double lon) async {
    List<Places> _place = await CommonService.instance.searchNearBy(lat: lat, lon: lon);
    places.addAll(_place);
  }
}
...
class LocationErrorWidget extends StatelessWidget {
  ...
  const LocationErrorWidget({super.key, required this.showError, required this.onCompletion});

...
class DashboardScreenController extends BaseController with GetSingleTickerProviderStateMixin, WidgetsBindingObserver {
        ...
        AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const Text('Time for a Break!', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
          content: const Text('You\'ve been using the app for 30 minutes. Consider taking a short break to rest your eyes and stretch.'),
          actions: [
            TextButton(
              onPressed: () {
              ...
              child: Text('Got it', style: TextStyle(color: Theme.of(Get.context!).primaryColor)),
            ),
          ],
        ),
      );
  ...
  void _fetchUnReadCount() {
    _unReadCountSubscription = db
        ...
        .snapshots()
        .listen((event) {
      final count = event.docs.where((doc) => (doc.data().msgCount ?? 0) > 0).length;

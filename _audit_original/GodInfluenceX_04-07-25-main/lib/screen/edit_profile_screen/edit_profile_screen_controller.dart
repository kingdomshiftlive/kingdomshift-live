...
class EditProfileScreenController extends BaseController {
  ...
  void initUserData() async {
    ...
    fullNameController = TextEditingController(text: userData.value?.fullname ?? '');
    usernameController = TextEditingController(text: userData.value?.username ?? '');
    bioController = TextEditingController(text: userData.value?.bio ?? '');
    emailController = TextEditingController(text: userData.value?.userEmail ?? '');
    phoneNumberController = TextEditingController(text: userData.value?.userMobileNo);
  ...
  void onChangeProfileImage() async {
    ...
    try {
      final XFile? image = await MediaPickerHelper.shared.pickImage(source: ImageSource.gallery);
      if (image == null) return;
      if (fileProfileImage.value != null) {
        File(fileProfileImage.value?.path ?? '').delete();
      }
      XFile? compressed = await MediaPickerHelper.shared.compressProfileImage(image.path);
      if (compressed != null) {
        Loggers.info("Compressed size: ${(await compressed.length()) / 1024} KB");
  ...
  void onSaveTap() async {
    ...
    User? userData = await UserService.instance.updateUserDetails(
        ...
        mobileCountryCode: int.parse(phoneController.selectedCode.value!.phoneCode.replaceAll('+', '')),
  ...
  void checkUsernameAvailability(String value) {
    ...
    if (username.isNotEmpty && currentUser?.toLowerCase() == username.toLowerCase()) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () async {
      final model = await UserService.instance.checkUsernameAvailability(userName: username);
            ...
            ConfirmationSheet(
              ...
              onTap: () async {
                ...
                LinksModel value = await UserService.instance
                    .addEditDeleteUserLink(linkType: LinkType.delete, linkId: link.id?.toInt());
                stopLoader();
  ...
  void openAddEditLinkSheet() {
    ...
    if (links.length >= limit) {
      return showSnackBar(LKey.maxUserLinkAddDescription.trParams({'limit': limit.toString()}));
    }
    Get.bottomSheet(
        AddEditLinksSheet(onLinksUpdate: (link) => onLinkAddEditDelete(link, LinkType.add), type: LinkType.add),
        isScrollControlled: true);
  }
}

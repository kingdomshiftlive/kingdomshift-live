...
class AddEditLinksSheet extends StatefulWidget {
  ...
  const AddEditLinksSheet({super.key, this.link, required this.onLinksUpdate, required this.type});
...
class _AddEditLinksSheetState extends State<AddEditLinksSheet> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            BottomSheetTopView(title: widget.link == null ? LKey.addLink.tr : LKey.editLink.tr),
            TextFieldCustom(
  ...
  void _onSave() async {
    ...
    switch (widget.type) {
      ...
      case LinkType.edit:
        Link link = (response.data ?? []).firstWhere((element) => element.id == widget.link?.id);

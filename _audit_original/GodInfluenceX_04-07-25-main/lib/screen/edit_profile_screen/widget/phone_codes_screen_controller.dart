...
class PhoneCodesScreenController extends BaseController {
  ...
  Future<void> loadCodes() async {
    ...
    if (editProfileController?.userData.value?.countryCode == null) {
      ...
      selectedCode.value = allCountries
          .firstWhereOrNull((element) => element.countryCode == editProfileController?.userData.value?.countryCode);
    }
  }

  Country? getPhoneCodeFromIP() {
    return allCountries.firstWhereOrNull((model) {
      return model.countryCode == PlatformDispatcher.instance.locale.countryCode;
    });
  }
}

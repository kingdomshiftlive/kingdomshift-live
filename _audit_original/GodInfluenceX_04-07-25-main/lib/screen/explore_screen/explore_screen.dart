            ...
            return MyRefreshIndicator(
              ...
              child: isLoading && exploreData == null
                  ...
                  : NoDataView(
                      ...
                      child:
                          SearchScreenGridView(postList: exploreData?.highPostHashtags ?? [], controller: controller),
                    ),
            );
          }),
        ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) => _buildHashtagItem(context, hashtags[index]),
      ),
    );
  }
...
class SearchScreenGridView extends StatelessWidget {
  ...
  List<Post> _preparePostList(HighPostHashtags highPostHashtags) {
    ...
    if (posts.length >= 5) {
      final reelPostIndex = posts.indexWhere((p) => p.postType == PostType.reel);
      if (reelPostIndex != -1) {
  ...
  Widget _buildHashtagHeader(BuildContext context, HighPostHashtags highPostHashtags) {
    return Padding(
  ...
  Widget _buildHashtagInfo(HighPostHashtags highPostHashtags, BuildContext context) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget _buildExploreButton(HighPostHashtags highPostHashtags, BuildContext context) {
    return InkWell(
  ...
  List<QuiltedGridTile> _getGridPattern(int postCount) {
    return [
      ...
      const QuiltedGridTile(1, 1),
      postCount <= 4 ? const QuiltedGridTile(1, 1) : const QuiltedGridTile(2, 1),
      const QuiltedGridTile(1, 1),
      const QuiltedGridTile(1, 1),
    ];
  }
  ...
  Widget _buildPostItem(BuildContext context, Post post) {
    final image = (post.postType == PostType.image && (post.images?.isNotEmpty ?? false)

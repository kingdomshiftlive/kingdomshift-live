...
class FeedScreenController extends BaseController {
  ...
  // Pagination state for discover posts
  RxInt currentDiscoverPage = 1.obs;
  RxBool hasMoreDiscoverPosts = true.obs;
  ...
  Future<void> fetchDiscoverPost({bool isEmpty = false}) async {
    if (isLoading.value || !hasMoreDiscoverPosts.value) return;
    isLoading.value = true;

    // Reset pagination if it's a fresh load
    if (isEmpty) {
      currentDiscoverPage.value = 1;
      hasMoreDiscoverPosts.value = true;
    }

    List<Post> _post =
        await PostService.instance.fetchPostsDiscover(type: PostType.posts, page: currentDiscoverPage.value);

    // Check if we got any posts
    if (_post.isEmpty) {
      hasMoreDiscoverPosts.value = false;
    } else {
  ...
  Future<void> _fetchPostsNearBy({bool isEmpty = false}) async {
    ...
    List<Post> _post = await PostService.instance
        .fetchPostsNearBy(type: PostType.posts, placeLat: position.latitude, placeLon: position.longitude);
  ...
  Future<void> onChangeCategory(PostCategory value) async {
    ...
    switch (value) {
      ...
      case PostCategory.podcast:
        // Reusing discover logic but with type=5 (handled by _fetchPodcasts if separate, or modification to fetchDiscoverPost)
        // Since user wants specific type=5, let's create _fetchPodcasts or reuse fetchDiscoverPost with type param if possible.
        // fetchDiscoverPost uses PostType.posts which is 1,2,3,4,5.
        // User wants ONLY 5.
        await _fetchPodcasts(isEmpty: true);
    }
  }
  ...
  Map<String, dynamic> getPaginationStatus() {
    return {
      'currentPage': currentDiscoverPage.value,
      'hasMorePosts': hasMoreDiscoverPosts.value,
      'totalPosts': posts.length,
      'category': selectedPostCategory.value.name,
    };
  }
  ...
  void onWatchStory(List<User> users, int index, String watchType) {
    Get.bottomSheet(
      StoryViewSheet(
        ...
        onUpdateDeleteStory: (story) {
          ...
          if (userId == SessionManager.instance.getUserID()) {
            ...
            if (userIndex != -1) {
              (stories[userIndex].stories ?? []).removeWhere((s) => s.id == storyId);
            }
          }
        },
      ),
      ...
      useRootNavigator: true, // Ensures the BottomSheet is on top of all navigators
    ).then((value) {
  ...
  Future<void> _loadMoreData() async {
    if (postScrollController.position.pixels >= (postScrollController.position.maxScrollExtent - 300) &&
        !isLoading.value) {
      switch (selectedPostCategory.value) {
        case PostCategory.discover:
          // Only load more if we have more posts available
          if (hasMoreDiscoverPosts.value) {
  ...
  Future<void> _fetchPodcasts({bool isEmpty = false}) async {
    if (isLoading.value || !hasMoreDiscoverPosts.value) return;
    isLoading.value = true;
    ...
    // Type 5 is Podcast
    List<Post> _post = await PostService.instance
        .fetchPostsDiscover(type: '${PostType.podcast.type}', page: currentDiscoverPage.value);

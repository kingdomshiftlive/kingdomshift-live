        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: !isStoryAvailable
                                ? Colors.transparent
                                : (isWatch ? disableGrey(context) : themeAccentSolid(context)),
                            width: 2,
                            strokeAlign: BorderSide.strokeAlignOutside),
                      ),
                      alignment: Alignment.center,
                      child: _buildImage(user?.profilePhoto?.addBaseURL() ?? '',
                          size: storySize + (!isStoryAvailable ? 3 : 0), fullname: user?.fullname),
                    ),
                    InkWell(
                      ...
                      child: Align(
                        ...
                        child: Container(
                          ...
                          child: Icon(Icons.add_rounded, color: whitePure(context), size: 18),
                        ),
                      ),
                    ),
                  ],
              ...
              Text(
                LKey.you.tr,
                style: TextStyleCustom.outFitRegular400(fontSize: 13, color: blackPure(context)),
              ),
            ],
          ),
        );
    ...
    Widget _buildOtherStory(List<User> users, int index) {
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            GradientBorder(
              gradient: isWatch ? StyleRes.disabledGreyGradient() : StyleRes.themeGradient,
              strokeWidth: 2,
              radius: 90,
              onPressed: () => controller.onWatchStory(users, index, 'other_story'),
              child: Padding(
                padding: const EdgeInsets.all(3.0),
                child: _buildImage(users[index].profilePhoto?.addBaseURL() ?? '',
                    size: storySize, fullname: users[index].fullname),
              ),
            ),
            Expanded(
              child: Text(

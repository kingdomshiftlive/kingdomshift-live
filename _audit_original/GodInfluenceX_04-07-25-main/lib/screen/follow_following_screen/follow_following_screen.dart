...
class FollowFollowingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(FollowFollowingScreenController(type, user, context), tag: '${user?.id}');
    return Scaffold(
      body: Column(
        children: [
          Container(
            ...
            child: SafeArea(
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      const CustomBackButton(width: 35, height: 18, alignment: AlignmentDirectional.centerStart),
                      CustomImage(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            FullNameWithBlueTick(username: user?.username ?? '', fontSize: 13),
                            Text(user?.fullname ?? '',
          ...
          Expanded(
            child: Obx(() {
              ...
              final showFollowersLoader = controller.isFollowers.value && isFollowersEmpty;
              final showFollowingsLoader = controller.isFollowings.value && isFollowingsEmpty;
              final showFollowersNoData = !controller.isFollowers.value && isFollowersEmpty;
              final showFollowingsNoData = !controller.isFollowings.value && isFollowingsEmpty;

              return PageView(
                controller: controller.pageController,
                onPageChanged: (value) => controller.selectedTabIndex.value = value,
                children: [
                  // Followers Page
                  _buildUserList(
                  ...
                  _buildUserList(
                    ...
                    noDataTitle: user?.showMyFollowing == 1 ? LKey.nothingToShowHere.tr : null,
                    noDataDescription: user?.showMyFollowing == 1 ? LKey.userHidFollowings.tr : null,
    ...
    return NoDataView(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isFollow = user?.isFollowing == null ? true : user?.isFollowing ?? false;
            ...
            Row(
              children: [
                ...
                if (widget.user?.id != SessionManager.instance.getUserID())
                  Obx(() {
                    Color textColor = widget.isFollowOrIsBlock ? textLightGrey(context) : whitePure(context);
                    return TextButtonCustom(
                      ...
                      title: widget.actionName == ActionName.follow
                          ? (widget.isFollowOrIsBlock ? LKey.unFollow.tr : LKey.follow.tr)
                          : (widget.isFollowOrIsBlock ? LKey.unBlock.tr : LKey.block.tr),
                      btnWidth: 100,
                      fontSize: 15,
                      horizontalMargin: 0,
                      btnHeight: 30,
                      ...
                      radius: 8,
                      backgroundColor: widget.isFollowOrIsBlock ? whitePure(context) : blueFollow(context),
                      borderSide: widget.isFollowOrIsBlock ? BorderSide(color: bgGrey(context)) : BorderSide.none,
                      child: isLoading.value ? CupertinoActivityIndicator(radius: 8, color: textColor) : null,
                    );
                  })
              ],
            ),

...
class FollowFollowingScreenController extends BaseController {
  ...
  FollowFollowingScreenController(this.followFollowingType, this.user, this.context) {
  ...
  void onInit() {
    super.onInit();
    selectedTabIndex.value = followFollowingType == FollowFollowingType.follower ? 0 : 1;
    _initData();
  }
  ...
  Future<void> fetchFollowers() async {
    ...
    List<Follower> list = await UserService.instance.fetchMyFollowers(lastItemId: lastId, userId: user?.id?.toInt());
    if (list.isNotEmpty) {
  ...
  Future<void> fetchFollowings() async {
    ...
    List<Following> list = await UserService.instance.fetchMyFollowing(lastItemId: lastId, userId: user?.id?.toInt());
    if (list.isNotEmpty) {
  ...
  void onChangeTab(int value) {
    ...
    pageController.animateToPage(value, duration: const Duration(milliseconds: 250), curve: Curves.linear);
  }
  ...
  Future<void> _handleFollowerAction(Follower data) async {
    ...
    if (Get.isRegistered<FollowController>(tag: userId.toString())) {
      ...
      followController = Get.put(FollowController(data.fromUser.obs), tag: userId.toString());
    }
  ...
  Future<void> _handleFollowingAction(Following data) async {
    ...
    if (Get.isRegistered<FollowController>(tag: userId.toString())) {
      ...
      followController = Get.put(FollowController(data.toUser.obs), tag: userId.toString());
    }

...
class SendGiftSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(SendGiftSheetController(giftType, userId, streamUsers));
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Column(
          children: [
            BottomSheetTopView(title: LKey.sendGifts.tr, margin: const EdgeInsets.only(top: 15)),
            switch (giftType) {
              GiftType.none => const SizedBox(),
              GiftType.livestream => GiftForLiveStream(controller: controller, streamUsers: streamUsers),
              GiftType.battle => battleViewType == BattleView.multi
                  ? GiftForLiveStream(controller: controller, streamUsers: streamUsers)
                  : Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(
                            LKey.youAreSendingCoinsTo.trParams({'color': battleViewType.value}),
                            style: TextStyleCustom.outFitLight300(color: whitePure(context), fontSize: 12),
                          )
                        ],
                      ),
                    ),
            ...
            Obx(() => GradientText((controller.myUser.value?.coinWallet ?? '0').toString(),
                gradient: StyleRes.themeGradient, style: TextStyleCustom.unboundedSemiBold600(fontSize: 21))),
            Text(LKey.coinsYouHave.tr,
                style: TextStyleCustom.outFitRegular400(fontSize: 15, color: textLightGrey(context))),
            const SizedBox(height: 10),
            Expanded(child: Obx(
              () {
                ...
                return GridView.builder(
                  ...
                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                      crossAxisCount: 4, mainAxisExtent: 126, crossAxisSpacing: 5, mainAxisSpacing: 5),
                          ...
                          children: [
                            if (gift.type == "video")
                              MediaDisplayWidget(
                                ...
                                strokeWidth: 0,
                                // Adjust if you want a border
                                isImageLoaderVisible: true,
                                isStokeOutSide: true,
                                fit: BoxFit.cover,
                              )
                            else
                              CustomImage(image: gift.image?.addBaseURL(), size: const Size(65, 65), radius: 0),
                            Text('${(gift.coinPrice ?? 0).numberFormat} ${LKey.coins.tr}',
                                style: TextStyleCustom.outFitMedium500(fontSize: 13, color: textLightGrey(context))),
...
class GiftForLiveStream extends StatelessWidget {
  ...
  const GiftForLiveStream({super.key, required this.controller, required this.streamUsers});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Obx(() {
          AppUser? giftUser = controller.livestreamController.selectedGiftUser.value;
          if (giftUser == null) {
            return const SizedBox();
          }
          return streamUsers.length <= 1
              ? Container(color: bgLightGrey(context), child: _PopupMenuItemCustom(streamUser: giftUser))
              : PopupMenuButton<AppUser>(
                  ...
                  shape: const RoundedRectangleBorder(
                    borderRadius:
                        SmoothBorderRadius.vertical(bottom: SmoothRadius(cornerRadius: 15, cornerSmoothing: 1)),
                  ),
                  ...
                  itemBuilder: (BuildContext context) {
                    return List.generate(
                      ...
                      (index) => PopupMenuItem(
                          ...
                          child: _PopupMenuItemCustom(streamUser: streamUsers[index])),
                    );
                  },
                  child: _PopupMenuItemCustom(isPopupChild: true, streamUser: giftUser),
                );
        }),
...
class _PopupMenuItemCustom extends StatelessWidget {
  ...
  const _PopupMenuItemCustom({this.isPopupChild = false, required this.streamUser});
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (isPopupChild) Image.asset(AssetRes.icDownArrow_1, width: 26, height: 26),
        ],
      ),
    );
  }

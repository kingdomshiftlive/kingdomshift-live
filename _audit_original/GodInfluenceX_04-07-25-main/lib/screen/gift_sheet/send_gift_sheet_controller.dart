...
class SendGiftSheetController extends BaseController {
  ...
  void onInit() {
    ...
    if (liveUsers.isNotEmpty && (giftType == GiftType.livestream || giftType == GiftType.battle)) {
      ...
      if (livestreamController.selectedGiftUser.value == null) {
        ...
        DebounceAction.shared.call(() {
          livestreamController.selectedGiftUser.value = liveUsers.firstWhere(
              (element) => element.userId == livestreamController.selectedGiftUser.value?.userId,
  ...
  Future<void> sendGift(Gift gift, BuildContext context) async {
    ...
    if (coinPrice <= 0) {
      return Loggers.error('Invalid coin price: $coinPrice, skipping gift sending.');
    }
    showLoader();
    final response = await GiftWalletService.instance.sendGift(giftId: giftId, userId: userId);
    stopLoader();
    if (response.status == true) {
      ...
      if (giftType == GiftType.none) {
        ...
        Get.back(result: GiftManager(gift, streamUser: livestreamController.selectedGiftUser.value));
      }
    } else {

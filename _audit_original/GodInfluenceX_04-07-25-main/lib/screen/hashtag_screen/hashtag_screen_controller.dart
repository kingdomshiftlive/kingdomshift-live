...
class HashtagScreenController extends BaseController {
  ...
  Future<List<Post>> fetchReels() async {
    ...
    HashtagPostData? _post = await PostService.instance.fetchPostsByHashtag(
        type: PostType.reels, hashTag: hashTag.removeHash, lastItemId: reels.post.lastOrNull?.id?.toInt());
  ...
  Future<List<Post>> fetchPosts() async {
    ...
    HashtagPostData? _post = await PostService.instance.fetchPostsByHashtag(
        type: PostType.posts, hashTag: hashTag.removeHash, lastItemId: posts.post.lastOrNull?.id?.toInt());

...
class HomeScreenController extends BaseController with GetSingleTickerProviderStateMixin, WidgetsBindingObserver {
  ...
  Future<void> _handleDeepLink(Uri uri) async {
    ...
    if (queryParams.containsKey(Params.postId)) {
      try {
        ...
        PostByIdModel model = await PostService.instance.fetchPostById(postId: postId);
        if (model.status == true) {
          ...
          if (post != null) {
            ...
            } else if ([PostType.image, PostType.video, PostType.text].contains(post.postType)) {
              await Get.to(() => SinglePostScreen(post: post, isFromNotification: true));
            }
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (liveStreamId.isNotEmpty) {
          DocumentSnapshot<Map<String, dynamic>> snapshot =
              await FirebaseFirestore.instance.collection(FirebaseConst.liveStreams).doc(liveStreamId).get();
          if (snapshot.exists && snapshot.data() != null) {
            ...
            await Get.to(() => LiveStreamAudienceScreen(livestream: livestream, isHost: false));
          }
        }
      } catch (e) {
  ...
  Future<void> fetchDiscoverPost(bool resetData) async {
    List<Post> newPosts = await PostService.instance.fetchPostsDiscover(type: PostType.reels, page: currentPage.value, cancelToken: token);
  ...
  Future<void> _fetchFollowingPost(bool resetData) async {
    ...
    addResponseData(newPosts, resetData);
    hasMoreData.value = false; // Disable pagination for following if api doesn't support
  }
  ...
  Future<void> _fetchPostsNearBy(bool resetData) async {
    ...
    List<Post> newPosts = await PostService.instance.fetchPostsNearBy(
        type: PostType.reels, placeLat: position.latitude, placeLon: position.longitude, cancelToken: token); // API doesn't have pagination yet
    addResponseData(newPosts, resetData);

...
class LiveStreamSummary extends StatelessWidget {
  ...
  const LiveStreamSummary({super.key, this.userState, required this.isHost, required this.viewers, this.onGoHomeTap});
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      ...
                      style: TextStyleCustom.unboundedRegular400(fontSize: 20, color: whitePure(context)),
                    ),
                  ),
                  Text(
                    LKey.belowIsTheSummaryOfYourStream.tr,
                    style: TextStyleCustom.outFitThin100(fontSize: 17, color: textLightGrey(context)),
                  ),
                ],
              ),
              Column(
                children: [
                  BuildTextAndValueTiles(
                    ...
                    value: (userState?.joinStreamTime ?? DateTime.now().millisecondsSinceEpoch).elapsedTimeFromEpoch,
                  ),
                  BuildTextAndValueTiles(title: LKey.viewers.tr, value: '${(viewers - 1).clamp(0, viewers)}'),
                  BuildTextAndValueTiles(
...
class BuildTextAndValueTiles extends StatelessWidget {
  ...
  const BuildTextAndValueTiles({super.key, this.widget, required this.title, required this.value});
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                title,
                style: TextStyleCustom.outFitLight300(color: whitePure(context).withValues(alpha: .7), fontSize: 16),
              ),
              Text(
                value,
                style: TextStyleCustom.outFitMedium500(color: whitePure(context).withValues(alpha: .7), fontSize: 18),
              ),
            ],
          ),

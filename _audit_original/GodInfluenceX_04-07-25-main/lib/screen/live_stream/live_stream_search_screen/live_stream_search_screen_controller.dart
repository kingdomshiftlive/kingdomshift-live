...
class LiveStreamSearchScreenController extends BaseController {
  ...
  void onLiveUserTap(Livestream stream) async {
    ...
    // Check if this is an audio livestream
    // - New audio streams have type == audioLivestream
    // - Existing/legacy audio streams have type == livestream but hostViewID == -1 (no video view)
    bool isAudioStream = stream.type == LivestreamType.audioLivestream ||
        (stream.type == LivestreamType.livestream && stream.hostViewID == -1);

    if (isAudioStream) {
      bool isHost = stream.hostId == myUser?.id;
      Get.to(() => AudioLiveStreamScreen(isHost: isHost, liveData: stream));
      return;
    }
    ...
    try {
      ...
      for (var dummy in dummyLives) {
        ...
        if (!alreadyExists && dummy.status == 1) {
          ...
          Loggers.info('ℹ️ No action for: $dummyId (exists: $alreadyExists, status: ${dummy.status})');
        }
      }
    } catch (e, _) {

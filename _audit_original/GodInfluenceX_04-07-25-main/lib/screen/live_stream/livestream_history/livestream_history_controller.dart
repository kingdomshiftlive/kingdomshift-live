...
class LivestreamHistoryController extends BaseController {
  ...
  final String apiUrl = 'https://admin.godinfluencex.com/api/recording-history/';
  final String deleteApiUrl = 'https://admin.godinfluencex.com/api/delete-recording-history/';
...
// Register callback only if flutter_downloader is initialized
  ...
  Future<void> fetchLivestreams(String userId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (livestreamHistory.status == true && livestreamHistory.history != null) {
  ...
  Future<void> deleteLivestreams(int? videoId) async {
    try {
      showLoader();
      final response = await http.get(Uri.parse(deleteApiUrl + videoId.toString()));
      if (response.statusCode == 200) {
  ...
  static void downloadCallback(String id, int status, int progress) {
    ...
    print('download test callback = id: $id, status: $status, progress: $progress');
      ...
      Get.dialog(
        AlertDialog(
          ...
          content: Obx(() => Column(
                ...
                children: [
                  LinearProgressIndicator(value: controller.downloadProgress[id] ?? 0.0),
                  const SizedBox(height: 10),
                  Text('${(controller.downloadProgress[id]! * 100).toStringAsFixed(0)}% complete'),
                ],
              )),
          actions: [
            TextButton(
  ...
  Future<void> showPermissionDialog() async {
    await Get.defaultDialog(
      ...
      middleText: 'Storage permission is permanently denied.\nPlease enable it from settings.',
  ...
  String getFullVideoUrl(String? relativeUrl) => relativeUrl != null ? videoBaseUrl + relativeUrl : '';
}

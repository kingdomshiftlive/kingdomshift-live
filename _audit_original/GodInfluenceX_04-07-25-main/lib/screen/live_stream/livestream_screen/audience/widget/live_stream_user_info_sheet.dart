...
class LiveStreamUserInfoSheet extends StatefulWidget {
  ...
  const LiveStreamUserInfoSheet({super.key, required this.isAudience, this.liveUser, required this.controller});

  @override
  State<LiveStreamUserInfoSheet> createState() => _LiveStreamUserInfoSheetState();
}
...
class _LiveStreamUserInfoSheetState extends State<LiveStreamUserInfoSheet> {
  ...
  _fetchUserProfile() async {
    ...
    user.value = await UserService.instance.fetchUserDetails(userId: widget.liveUser?.userId);
  ...
  Future<void> followUnFollowUser() async {
    ...
    if (Get.isRegistered<FollowController>(tag: userId.toString())) {
      ...
      followController = Get.put(FollowController(user), tag: userId.toString());
    }

    User? updateUser = await followController.followUnFollowUser();
    widget.controller.updateUserStateToFirestore(userId, isFollow: updateUser?.isFollowing);
        ...
        Container(
          ...
          decoration: ShapeDecoration(
            shape: const SmoothRectangleBorder(
              borderRadius: SmoothBorderRadius.vertical(top: SmoothRadius(cornerRadius: 40, cornerSmoothing: 1)),
            ),
                  ...
                  : Stack(
                      ...
                      children: [
                          ...
                          child: Column(
                            children: [
                              ...
                              FullNameWithBlueTick(
                                  username: user?.username, isVerify: user?.isVerify, fontSize: 14, iconSize: 18),
                              Text(
                                user?.fullname ?? '',
                                style: TextStyleCustom.outFitRegular400(fontSize: 16, color: textLightGrey(context)),
                              ),
                              if ((user?.bio ?? '').isNotEmpty) const SizedBox(height: 18),
                              ...
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: List.generate(statItems.length, (index) {
                              ...
                              if (widget.liveUser?.userId != SessionManager.instance.getUserID())
                                Row(
                                  spacing: 10,
                                  children: [
                                        ...
                                        child: TextButtonCustom(
                                          onTap: () {
                                            ...
                                            Get.bottomSheet(ConfirmationSheet(
                                                ...
                                                description: LKey.ifYouCheckThisProfileEtc.tr,
                                                onTap: () {
                                                  Get.back();
                                    ...
                                    Expanded(
                                      child: isFollowUnFollowInProcess.value
                                          ...
                                          : TextButtonCustom(
                                              ...
                                              title: isFollow ? LKey.unFollow.tr : LKey.follow.tr,
                                              titleColor: isFollow ? textLightGrey(context) : whitePure(context),
                                              backgroundColor: isFollow ? bgGrey(context) : blueFollow(context),
                                              horizontalMargin: 0),
                                    ),
                                  ],
                                ),
                              ...
                              if (!widget.isAudience)
                                Obx(() {
                                  LivestreamUserState? state = widget.controller.liveUsersStates.firstWhere(
                                    (element) {
                                      return element.userId == widget.liveUser?.userId;
                                    },
                                  );
                                  ...
                                  return Row(
                                    ...
                                    children: [
                                      LiveStreamCircleBorderButton(
                                        image: state.audioStatus != VideoAudioStatus.on
                                            ? AssetRes.icMicOff
                                            : AssetRes.icMicrophone,
                                        iconColor: textLightGrey(context),
                                        borderColor: bgGrey(context),
                                        size: const Size(40, 40),
                                        onTap: () => widget.controller.coHostAudioToggle(state),
                                      ),
                                      LiveStreamCircleBorderButton(
                                        image: state.videoStatus != VideoAudioStatus.on
                                            ? AssetRes.icVideoOff
                                            : AssetRes.icVideoCamera,
                                        iconColor: textLightGrey(context),
                                        borderColor: bgGrey(context),
                                        size: const Size(40, 40),
                                        onTap: () => widget.controller.coHostVideoToggle(state),
                                      ),
                                      Obx(() {
                                        final isMainUser =
                                            widget.controller.liveData.value.mainUserId == widget.liveUser?.userId;
                                        return InkWell(
                                          onTap: () {
                                            Get.back();
                                            if (isMainUser) {
                                              // Reset to host
                                              widget.controller.assignMainUser(null);
                                            } else {
                                              widget.controller.assignMainUser(widget.liveUser?.userId);
                                            }
                                          },
                                          child: Container(
                                            height: 40,
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              border: Border.all(
                                                color: isMainUser ? ColorRes.likeRed : bgGrey(context),
                                              ),
                                            ),
                                            child: Icon(
                                              Icons.star_rounded,
                                              size: 22,
                                              color: isMainUser ? ColorRes.likeRed : textLightGrey(context),
                                            ),
                                          ),
                                        );
                                      }),
                        ...
                        const CustomBackButton(
                            image: AssetRes.icClose, width: 23, height: 23, padding: EdgeInsets.all(10))
                      ],
                    );
            }),
          ),

...
class LiveStreamAudienceTopView extends StatelessWidget {
  ...
  const LiveStreamAudienceTopView({super.key, this.isAudience = false, required this.controller});
      ...
      children: [
        Obx(
          () {
            ...
            bool isAudience = stream.coHostIds?.contains(controller.myUserId) == false;
            ...
            return InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    border: Border.all(color: whitePure(context).withValues(alpha: .5), width: 1.5)),
                alignment: Alignment.center,
                child: Image.asset(AssetRes.icClose1,
                    color: whitePure(context).withValues(alpha: .5), width: 18, height: 18),
              ),
            );
          },
        ),
        Row(
          children: [
            InkWell(
              onTap: () {
                BranchIoManager.instance
                    .shareContent(type: ShareBranchType.liveStream, livestream: controller.liveData.value);
              },
              child: Image.asset(AssetRes.icPostShare,
                  color: whitePure(context).withValues(alpha: 0.5), width: 28, height: 28),
            ),
...
class _BuildCenterView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      Livestream stream = controller.liveData.value;
      AppUser? hostUser =
          controller.firestoreController.users.firstWhereOrNull((element) => element.userId == stream.hostId);
      return Row(
        spacing: 10,
        children: <Widget>[
          InkWell(
            onTap: () {
              Get.bottomSheet(
                LiveStreamUserInfoSheet(isAudience: true, liveUser: hostUser, controller: controller),
          ...
          Obx(() {
            ...
            bool isCoHost = (stream.coHostIds ?? []).contains(controller.myUserId);
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: whitePure(context).withValues(alpha: 0.3)),
                  ),
                  child: Row(
                    children: [
                ...
                if (!isBattleOn)
                  LiveStreamCircleBorderButton(
                    ...
                    iconColor: whitePure(context),
                    onTap: () {
                      Get.bottomSheet(const MembersSheet(isHost: false), isScrollControlled: true);
                    },
                  )
              ],
            );
...
class _BuildBottomView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      StreamView? hostView = controller.streamViews.firstWhereOrNull((element) => element.streamId == '${data.hostId}');
      if (isBattleView) {
      ...
      return Container(
        ...
        child: MuteUnMuteButton(
            isMute: isDummyLive ? controller.isPlayerMute : (hostView?.isMuted ?? false).obs,
            onTap: () =>
                isDummyLive ? controller.togglePlayerAudioToggle() : controller.toggleStreamAudio(data.hostId)),
      );
    });
  }
}

...
import 'package:shortzz/screen/live_stream/livestream_screen/view/multi_battle_view.dart';
...
class BattleView extends StatefulWidget {
  ...
  const BattleView({super.key, this.isAudience = false, required this.controller, this.margin});
...
class _BattleViewState extends State<BattleView> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (widget.controller.liveData.value.battleMode == BattleMode.multiUser) {
        return Stack(
          children: [
            LivestreamView(
            ...
            Column(
              children: [
                MultiBattleView(controller: widget.controller, margin: widget.margin),
                Obx(() => BattleTimer(controller: widget.controller, livestream: widget.controller.liveData.value)),
              ],
            ),
          ],
        );
      ...
      return SizedBox(
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              LiveBattleOverlayWidget(controller: widget.controller, margin: widget.margin),
              Obx(() => BattleTimer(controller: widget.controller, livestream: widget.controller.liveData.value)),
            ],
          ),
        ),
...
class LiveBattleOverlayWidget extends StatefulWidget {
  ...
  const LiveBattleOverlayWidget({super.key, required this.controller, this.margin});

  @override
  State<LiveBattleOverlayWidget> createState() => _LiveBattleOverlayWidgetState();
}
...
class BuildStates extends StatelessWidget {
  ...
  const BuildStates({super.key, required this.red, required this.blue, required this.users, required this.stream});
            ...
            children: [
              ...
              if (stream.battleType == BattleType.end)
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    _buildWinnerTag(context, rightSide: false, winnerTag: isRedWin),
                    _buildWinnerTag(context, rightSide: true, winnerTag: !isRedWin),
                  ],
                ),
  ...
  Widget _buildCoinStats(BuildContext context, {required bool topRight, required int coin}) {
    return Container(
      height: 26,
      ...
      decoration: ShapeDecoration(
        ...
        shape: SmoothRectangleBorder(
          borderRadius: topRight
              ? const SmoothBorderRadius.only(topRight: SmoothRadius(cornerRadius: 40, cornerSmoothing: 0))
              : const SmoothBorderRadius.only(topLeft: SmoothRadius(cornerRadius: 40, cornerSmoothing: 0)),
        ),
      ),
      ...
      child: Row(
        ...
        spacing: 5,
        mainAxisAlignment: topRight ? MainAxisAlignment.start : MainAxisAlignment.end,
        children: [
  ...
  Widget _buildWinnerTag(BuildContext context, {required bool rightSide, required bool winnerTag}) {
    return Expanded(
      child: Container(
        height: 31,
        padding: const EdgeInsets.symmetric(horizontal: 10),
        alignment: rightSide ? AlignmentDirectional.centerEnd : AlignmentDirectional.centerStart,
        decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              Colors.transparent,
                ...
                begin: !rightSide ? AlignmentDirectional.centerEnd : AlignmentDirectional.centerStart,
                end: !rightSide ? AlignmentDirectional.centerStart : AlignmentDirectional.centerEnd)),
        child: Text(
          (winnerTag ? LKey.victory.tr : LKey.defeat.tr).toUpperCase(),
          style: TextStyleCustom.unboundedBlack900(color: winnerTag ? ColorRes.green1 : ColorRes.likeRed, fontSize: 17),
        ),
      ),
    );
  }

  Widget _buildStreamerInfo(BuildContext context, {required bool isLeft, required AppUser user}) {
    return Expanded(
      child: Container(
        height: 43,
          ...
          children: [
            ...
            Flexible(
              child: Text(user.username ?? '',
                  style: TextStyleCustom.unboundedMedium500(color: whitePure(context), fontSize: 11),
                  maxLines: 1,
...
class _BuildLastTenSecondViewState extends State<BuildLastTenSecondView> with SingleTickerProviderStateMixin {
            ...
            builder: (context, child) => Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: SweepGradient(
                    colors: <Color>[whitePure(context).withValues(alpha: 0), whitePure(context).withValues(alpha: .5)],
                    transform: GradientRotation(2 * pi * _animationController.value)),
              ),
              child: AnimatedSwitcher(
                ...
                child: Text('$leftSecond',
                    style: TextStyleCustom.unboundedBlack900(color: whitePure(context), fontSize: 100),
...
class BattleTimer extends StatefulWidget {
  ...
  const BattleTimer({super.key, required this.controller, required this.livestream});
          ...
          child: Container(
            ...
            child: Text(
              ...
              style: TextStyleCustom.unboundedMedium500(
                  color: isBattleEnd ? ColorRes.likeRed : themeAccentSolid(context), fontSize: 18),
            ),
          ),
        ),
      );

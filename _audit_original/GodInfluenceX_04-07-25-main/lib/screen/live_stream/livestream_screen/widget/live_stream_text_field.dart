...
class LiveStreamTextFieldView extends StatelessWidget {
  ...
  const LiveStreamTextFieldView({super.key, required this.isAudience, required this.controller});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      bool isGiftIconVisible =
          controller.streamViews.firstWhereOrNull((view) => view.streamId == controller.myUserId.toString()) == null;

      List<AppUser> users = stream.getAllUsers(controller.firestoreController.users);
      ...
      return Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            isCollapsed: true,
            hintText: isAudience ? '${LKey.writeHere.tr}..' : LKey.whatDoYouThink.tr,
            hintStyle: TextStyleCustom.outFitLight300(color: whitePure(context), fontSize: 17, opacity: .42),
...
class TextFieldSuffixIcon extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bool showGiftIconsForBattle = isTextEmpty && isAudience && isBattleOn && users.length == 2;

    final bool showMultiBattleGiftIcon = isTextEmpty && isAudience && isBattleOn && users.length > 2;
    ...
    return AnimatedContainer(
      ...
      child: !isTextEmpty
          ...
          : showGiftIconsForBattle
              ...
              : showMultiBattleGiftIcon
                  ? GiftIcon(
                      bgColor: ColorRes.themeGradient1,
                      onTap: () => controller.onGiftTap(
                        GiftType.battle,
                        battleViewType: BattleView.multi,
                        users: users,
                      ),
                    )
                  : showSendButton && isTextEmpty && !isAudience
                      ...
                      : isBattleOn && isAudience
                          ...
                          : !isTextEmpty
                              ? _sendButton(context)
                              : GiftIcon(
                                  onTap: () => controller.onGiftTap(GiftType.livestream, users: users),
                                ),
    );
  }

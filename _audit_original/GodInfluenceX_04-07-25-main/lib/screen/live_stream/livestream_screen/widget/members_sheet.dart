...
class _MembersSheetState extends State<MembersSheet> {
  ...
  void onSelectedTab(int index) {
    ...
    pageController.animateToPage(index, duration: const Duration(milliseconds: 250), curve: Curves.linear);
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Obx(() {
        return Column(
          children: [
            BottomSheetTopView(title: LKey.members.tr, sideBtnVisibility: false),
            if (widget.isHost)
              CustomTabSwitcher(
                items: [LKey.requests.tr, LKey.audience.tr, LKey.invited.tr, LKey.coHosts.tr],
                onTap: onSelectedTab,
                ...
                : CustomSearchTextField(
                    ...
                    onChanged: (value) {
                      DebounceAction.shared.call(() {
                        ...
                        itemList = controller.liveUsersStates.search(value, (p0) {
                          AppUser? data = p0.getUser(controller.firestoreController.users);
                          return data?.username ?? '';
                        }, (p1) {
                          AppUser? data = p1.getUser(controller.firestoreController.users);
                          return data?.fullname ?? '';
                        });
                        if (widget.isHost) {
                          if (selectedTab.value == 0) {
                            controller.requestList.value =
                                itemList.where((element) => element.type == LivestreamUserType.requested).toList();
                          } else if (selectedTab.value == 1) {
                            controller.audienceList.value = itemList
                                .where((element) =>
                                    element.type != LivestreamUserType.host && element.type != LivestreamUserType.left)
                                .toList();
                          }
                        } else {
                          controller.audienceMemberList.value =
                              itemList.where((element) => element.type != LivestreamUserType.left).toList();
                        }
                      }, milliseconds: 500);
                    },
                  )),
                  ...
                  ? NoDataView(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final bool isInvited = state.type == LivestreamUserType.invited;
                          return MemberProfileCard(user: user, widget: _buildActionWidget(state, user, isInvited));
                        },
                      ),
                    )
                  : PageView(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return MemberProfileCard(
                                user: user,
                                widget: _buildActionWidget(state, user, isInvited),
                              );
                            },
                          ),
                        ),
  ...
  Widget _buildActionWidget(LivestreamUserState state, AppUser? user, bool isInvited) {
    ...
    switch (state.type) {
      ...
      case LivestreamUserType.coHost:
        return Row(
          children: [
            _buildActionBtn(
              state.videoStatus == VideoAudioStatus.on ? AssetRes.icVideoCamera : AssetRes.icVideoOff,
              textLightGrey(context),
              () => controller.coHostVideoToggle(state),
            ),
            _buildActionBtn(
              state.audioStatus == VideoAudioStatus.on ? AssetRes.icMicrophone : AssetRes.icMicOff,
              textLightGrey(context),
              () => controller.coHostAudioToggle(state),
            ),
            _buildActionBtn(AssetRes.icDelete1, ColorRes.likeRed, () => controller.coHostDelete(state)),
          ],
        );
      default:
...
class MemberProfileCard extends StatelessWidget {
  ...
  const MemberProfileCard({super.key, required this.widget, required this.user});
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              CustomImage(size: const Size(40, 40), image: user?.profile?.addBaseURL(), fullName: user?.fullname),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(user?.fullname ?? '', style: TextStyleCustom.outFitLight300(color: textLightGrey(context)))
                  ],
                ),
              ),
              widget
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: bgColor, shape: BoxShape.circle, border: Border.all(color: color)),
...
class TextBorderButton extends StatelessWidget {
  ...
  const TextBorderButton({super.key, required this.text, this.onTap, this.textOpacity});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Text(
          text,
          style: TextStyleCustom.outFitRegular400(color: textLightGrey(context), fontSize: 15, opacity: textOpacity),
        ),
      ),
    );
  }

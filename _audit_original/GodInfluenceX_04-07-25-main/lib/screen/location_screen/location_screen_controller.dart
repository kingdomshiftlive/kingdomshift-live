...
class LocationScreenController extends BaseController {
  ...
  int? _getMinPostId(List<Post> items) {
    return items.isEmpty ? null : items.map((p) => p.id ?? 0).reduce((a, b) => a < b ? a : b);
  }
  ...
  Future<void> onMapTap(LatLng latLng) async {
    ...
    googleMapController.future.then((value) {
      value.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: latLng, zoom: 14.4746)));
    });
  ...
  Future<void> _createMarker(LatLng latLng) async {
    ...
    // Update the marker
    marker[markerId] =
        Marker(markerId: markerId, position: LatLng(latLng.latitude, latLng.longitude), icon: customIcon);
  }
}

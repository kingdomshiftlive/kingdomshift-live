...
class MusicSheetController extends BaseController {
  ...
  onChangedMusicCategories(int index) {
    ...
    pageController.animateToPage(index, duration: const Duration(milliseconds: 250), curve: Curves.easeIn);
  ...
  void fetchMusicExplore({bool isEmpty = false}) async {
    ...
    List<Music> items = await PostService.instance.fetchMusicExplore(lastItemId: lastItemId);
    if (isEmpty) {
  ...
  void fetchMusicByCategories(int? categoryId, {bool isEmpty = false}) async {
    ...
    List<Music> items = await PostService.instance.fetchMusicByCategories(lastItemId: lastItemId, categoryId: id);
    if (isEmpty) {
  ...
  void updateSavedMusicIds(List<int> savedMusicIds) async {
    ...
    this.savedMusicIds.value =
        (SessionManager.instance.getUser()?.savedMusicIds ?? '').split(',').map((e) => int.parse(e)).toList();
  }
  ...
  void onTapMusic(Music music, bool isCategorySheet) async {
    ...
    String downloadMusicPath = (await DefaultCacheManager().downloadFile(music.sound?.addBaseURL() ?? '')).file.path;
    ...
    if (downloadMusicPath.isNotEmpty) {
      SelectedMusic? selectedMusic = await Get.bottomSheet<SelectedMusic>(
          SelectedMusicSheet(
              selectedMusic: SelectedMusic(music, 0, downloadMusicPath, 0), totalVideoSecond: videoSecond),
          enableDrag: false,

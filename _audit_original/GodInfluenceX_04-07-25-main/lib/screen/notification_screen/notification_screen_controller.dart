...
class NotificationScreenController extends BaseController {
  ...
  RxList<AdminNotificationData> adminNotifications = <AdminNotificationData>[].obs;
  RxList<ActivityNotification> activityNotifications = <ActivityNotification>[].obs;
  ...
  Future<void> fetchAdminNotification() async {
    ...
    List<AdminNotificationData> items =
        await NotificationService.instance.fetchAdminNotifications(lastItemId: adminNotifications.lastOrNull?.id);
  ...
  Future<void> fetchActivityNotifications() async {
    ...
    List<ActivityNotification> items =
        await NotificationService.instance.fetchActivityNotifications(lastItemId: activityNotifications.lastOrNull?.id);
  ...
  void onPostTap(ActivityNotification? data) async {
    ...
    showLoader();
    final PostByIdModel result =
        await PostService.instance.fetchPostById(postId: post!.id!, commentId: commentId, replyId: replyCommentId);
    stopLoader();
    ...
    } else if ([PostType.image, PostType.video, PostType.text].contains(postType)) {
      Get.to(() => SinglePostScreen(post: fetchedPost, postByIdData: result.data, isFromNotification: true));
    }
  }

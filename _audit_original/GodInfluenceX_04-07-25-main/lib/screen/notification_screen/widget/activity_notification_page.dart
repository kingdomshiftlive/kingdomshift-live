...
class ActivityNotificationPage extends StatelessWidget {
  ...
  const ActivityNotificationPage({super.key, required this.data, required this.controller});
  ...
  String _getNotificationText() {
    ...
    final replyCommentDesc = (data.data?.reply ?? data.data?.comment)?.commentDescription ?? '';
    ...
    switch (data.type) {
      ...
      case ActivityNotifyType.notifyCommentPost:
        ...
        return LKey.activityCommentedPost.trParams({'comment_description': commentDesc});
      ...
      case ActivityNotifyType.notifyMentionReply:
        return LKey.notifyReplyMentionedInComment.trParams({'comment_description': commentDesc});
  ...
  Widget? _buildPostOrGiftImage(BuildContext context) {
    ...
    if ([
      ...
      return CustomImage(
          ...
          image: (post?.postType == PostType.image ? post?.images?.first.image : post?.thumbnail)?.addBaseURL(),
...
enum ActivityNotifyType {
  ...
  static ActivityNotifyType fromString(int value) {
    return ActivityNotifyType.values
        .firstWhere((e) => e.type == value, orElse: () => throw ArgumentError('Invalid activity notify type : $value'));
  }
}

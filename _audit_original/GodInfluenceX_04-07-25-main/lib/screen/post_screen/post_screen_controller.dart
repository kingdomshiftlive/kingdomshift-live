...
class PostScreenController extends BaseController {
  ...
  void handleBlock(Post? post) {
    if (post?.user == null) return;
    BlockUserController().blockUser(post!.user, () {
      if (Get.isRegistered<ProfileScreenController>(tag: ProfileScreenController.tag)) {
        final controller = Get.find<ProfileScreenController>(tag: ProfileScreenController.tag);
        controller.posts.removeWhere((element) => element.userId == post.user!.id);
      }
    });
  }

  void handleTransparency(Post? post) {
    if (post == null) return;
    Get.bottomSheet(
      Container(
        ...
        child: Column(
          ...
          children: [
            Text("Why am I seeing this?", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Text("You are seeing this post because it is algorithmic trending or you follow the author.", 

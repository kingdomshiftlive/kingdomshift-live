...
class PostViewActionButton extends StatelessWidget {
  ...
  const PostViewActionButton(
      {super.key, required this.post, required this.controller, required this.likeKey, this.onTriggerReady});
      ...
      child: Wrap(
        children: [
          PostViewIconWithCount(
              ...
              image: post.isLiked ?? false ? AssetRes.icFillHeart : AssetRes.icHeart,
              count: post.likes),
          if (post.canComment == 1)
            PostViewIconWithCount(onTap: controller.onComment, image: AssetRes.icPostComment, count: post.comments),
          PostViewIconWithCount(onTap: controller.handleShare, image: AssetRes.icPostShare, count: post.shares),
          PostViewIconWithCount(
              onTap: () => controller.onSaved(post),
              image: post.isSaved ?? false ? AssetRes.icFillBookmark : AssetRes.icPostBookmark,
              count: post.saves),
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        InkWell(
          onTap: onTap,
          child: Image.asset(key: likeKey, image, color: color ?? textDarkGrey(context), height: 23, width: 23),
        ),
        if (isCountVisible)
          SizedBox(
            width: 35,
            child: Text(
              ...
              style: TextStyleCustom.outFitRegular400(fontSize: 12.5, color: textDarkGrey(context)),
            ),
          )
      ],
    );

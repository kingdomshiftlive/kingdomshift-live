...
class PostViewInfoHeader extends StatelessWidget {
  ...
  const PostViewInfoHeader(
      {super.key, required this.post, required this.controller, required this.shouldShowPinOption});
  ...
  Widget build(BuildContext context) {
    ...
    if (Get.isRegistered<ProfileController>(tag: '${post.userId}')) {
      ...
      profileController = Get.put(ProfileController(post.user), tag: '${post.userId}');
    }
            ...
            children: [
              FullNameWithBlueTick(
                ...
                child: Text(
                    ...
                    style: TextStyleCustom.outFitLight300(color: textLightGrey(context), fontSize: 12)),
                onTap: () {
                ...
                InkWell(
                  onTap: () {
                    Get.to(() => LocationScreen(
                        latLng: LatLng(post.placeLat?.toDouble() ?? 0, post.placeLon?.toDouble() ?? 0),
  ...
  List<MenuItem> _getMenuItems(bool isModerator, Post post, PostScreenController controller) {
    final isMyPost = post.userId?.toInt() == SessionManager.instance.getUserID();
    if (isMyPost) {
      return [
        if (shouldShowPinOption)
          MenuItem(post.isPinned == 0 ? LKey.pin.tr : LKey.unpin.tr,
              () => controller.handlePinUnpinPost(post.isPinned ?? 0)),
        MenuItem(LKey.delete.tr, () => controller.handleDelete(post, isModerator: false)),
      ];
    } else {
      return [
        MenuItem(LKey.report.tr, () => controller.handleReport(post)),
        MenuItem("Why am I seeing this?", () => controller.handleTransparency(post)),
        MenuItem("Block User", () => controller.handleBlock(post)),
        if (isModerator) MenuItem(LKey.delete.tr, () => controller.handleDelete(post, isModerator: true)),
      ];
    }
  }
}

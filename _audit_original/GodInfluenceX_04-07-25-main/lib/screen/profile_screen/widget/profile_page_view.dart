...
import 'package:shortzz/screen/profile_screen/widget/podcast_list_view.dart';
  ...
  Widget build(BuildContext context) {
    return Expanded(child: Obx(() {
      ...
      return isUserNotFound
          ? NoDataView(title: LKey.noUserPostsTitle.tr, description: LKey.noUserPostsDescription.tr)
          : user?.isBlock == true
              ? const BlockUserView()
              : user?.isFreez == 1
                  ? const FreezeUser()
                  : PageView(
                      ...
                      children: [
                        ReelList(
                            ...
                            menus: isMe
                                ? [
                                    ContextMenuElement(title: '', onTap: controller.onPinUnpinReel),
                                    ContextMenuElement(
                                        title: LKey.delete.tr,
                                        onTap: (post) => controller.onDeleteReel(post, isModerator: false))
                                  ]
                                : [
                                    if (isModerator)
                                      ContextMenuElement(
                                          title: LKey.delete.tr,
                                          onTap: (post) => controller.onDeleteReel(post, isModerator: true))
                                  ],
                            isPinShow: true),
                        PostList(
                        ...
                        PodcastListView(
                            ...
                            isLoading: controller.isPodcastLoading,
                            onFetchMoreData: controller.fetchPodcast,
                            menus: isMe
                                ? [
                                    ContextMenuElement(title: '', onTap: controller.onPinUnpinReel),
                                    ContextMenuElement(

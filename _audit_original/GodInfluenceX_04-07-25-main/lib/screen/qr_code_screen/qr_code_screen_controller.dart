...
class QrCodeScreenController extends BaseController {
  ...
  Future<void> saveGalleryImage(String type) async {
    ...
    if (type == 'save') {
      ...
      final params = ShareParams(
        ...
        title: BranchIoManager.instance
            .getUserTitle(fullname: myUser.value?.fullname ?? '', username: myUser.value?.username ?? ''),
      );
      try {
  ...
  Future<void> fetchUserDetail() async {
    ...
    CustomBranchResponse? response =
        await BranchIoManager.instance.generateLink(type: ShareBranchType.user, user: myUser.value);

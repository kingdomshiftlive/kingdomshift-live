...
import 'package:shortzz/screen/reels_screen/reel/widget/side_bar_list_left.dart';
...
class ReelPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      return hasVideoInitialize
          ? ClipRRect(
              child: SizedBox.expand(
                child: FittedBox(
                  fit: (size?.width ?? 0) < (size?.height ?? 0) ? BoxFit.cover : BoxFit.fitWidth,
                  child: SizedBox(
                    width: size?.width ?? 0,
      ...
      return ValueListenableBuilder(
        ...
        builder: (context, value, child) {
          return !value.isInitialized || value.isBuffering
              ? const Center(child: CupertinoActivityIndicator(radius: 12, color: Colors.white))
              : AnimatedOpacity(
                  ...
                  child: Align(
                    ...
                    child: ClipRRect(
                      ...
                      child: Container(
                          ...
                          width: 60,
                          decoration:
                              BoxDecoration(color: blackPure(context).withValues(alpha: 0.5), shape: BoxShape.circle),
                          alignment: Alignment.center,
    ...
    void _handleVisibilityChanged(VisibilityInfo info) {
      ...
      if ((info.visibleFraction * 100) > 60) {
        // was 90
        videoPlayerController?.play();
      } else {
        videoPlayerController?.pause();
          ...
          child: InkWell(
            ...
            child: Stack(
              ...
              children: [
                ...
                if (videoPlayerController != null) _buildPlayPauseOverlay(videoPlayerController),
                Obx(() {
...
class ReelInfoRow extends StatelessWidget {
  ...
  const ReelInfoRow({super.key, required this.controller, required this.likeKey, required this.reelsScreenController});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        UserInformation(controller: controller),
      ],
    );
  }
}

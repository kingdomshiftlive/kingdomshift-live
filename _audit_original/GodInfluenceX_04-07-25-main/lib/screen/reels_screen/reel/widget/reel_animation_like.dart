...
class _ReelAnimationLikeState extends State<ReelAnimationLike> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scale = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0.0, end: 1.5).chain(CurveTween(curve: Curves.easeOut)), weight: 30),
      TweenSequenceItem(
        tween: Tween(begin: 1.5, end: 0.7).chain(CurveTween(curve: Curves.bounceIn)),
        weight: 100,
      ),
    ]).animate(_controller);
  ...
  void didChangeDependencies() {
    ...
    final RenderBox? box = widget._likeKey.currentContext?.findRenderObject() as RenderBox?;
    if (box == null) return;

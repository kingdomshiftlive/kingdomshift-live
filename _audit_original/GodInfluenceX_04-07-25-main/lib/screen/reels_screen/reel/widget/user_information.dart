...
class UserInfoHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (Get.isRegistered<ProfileController>(tag: '${reelUser?.id}')) {
      ...
      profileController = Get.put(ProfileController(reelUser), tag: '${reelUser?.id}');
    }
...
class _FollowButtonState extends State<FollowButton> {
  @override
  Widget build(BuildContext context) {
    final followController = Get.put(FollowController(widget.controller.reelData.value.user.obs),
        tag: '${widget.controller.reelData.value.userId}');
    RxBool isLoading = false.obs;
    return Obx(() {
      bool isFollow = followController.user.value?.isFollowing ?? false;
      if (followController.user.value?.id == SessionManager.instance.getUserID()) {
        ...
        child: InkWell(
          ...
          child: isLoading.value
              ? Padding(
                  ...
                  child: CupertinoActivityIndicator(radius: 8.5, color: whitePure(context).withValues(alpha: .3)))
              : Container(
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      return Row(
        children: [
          Text(
            reel.id == -1 ? DateFormat('dd MMM yyyy').format(DateTime.now()) : (reel.createdAt ?? '').formatDate2,
            style: TextStyleCustom.outFitLight300(color: whitePure(context), opacity: .8, fontSize: 11),
          ),
          // if (views > 0)
          //   Row(
          //     children: [
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        ...
        Get.to(() => LocationScreen(latLng: latLng, placeTitle: reel.placeTitle ?? ''), preventDuplicates: false);
      },
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          ConstrainedBox(
            ...
            child: SingleChildScrollView(
              child: ReadMoreText(
                ...
                style: TextStyleCustom.outFitLight300(color: whitePure(context), opacity: .8, fontSize: 15),
                  ...
                  Annotation(
                    ...
                    spanBuilder: ({required String text, TextStyle? textStyle}) => TextSpan(
                        ...
                        recognizer: TapGestureRecognizer()
                          ..onTap = () async {
                            await Get.to(() => HashtagScreen(hashtag: text, index: 0), preventDuplicates: false);
                          }),
                  ),
                  Annotation(
                    ...
                    spanBuilder: ({required String text, TextStyle? textStyle}) {
                      return TextSpan(
                        ...
                        recognizer: TapGestureRecognizer()
                          ..onTap = () async {
                            User? user = widget.controller.reelData.value.mentionedUsers?.firstWhereOrNull((element) {
                              return element.username == text.replaceAll('@', '');
                            });
                            if (user != null) {
                ...
                trimExpandedText: '   ',
                delimiter: '',
                moreStyle: TextStyleCustom.outFitLight300(color: whitePure(context), opacity: .8, fontSize: 15),
                lessStyle: TextStyleCustom.outFitLight300(color: whitePure(context), opacity: .8, fontSize: 15),
              ),
            ),
          ),
        ],

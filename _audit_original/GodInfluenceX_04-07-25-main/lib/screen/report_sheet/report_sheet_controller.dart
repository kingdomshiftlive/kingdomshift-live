...
class ReportSheetController extends BaseController {
  List<ReportReason> reports = SessionManager.instance.getSettings()?.reportReason ?? [];
  ...
  void _reportPost() async {
    ...
    StatusModel model = await PostService.instance.reportPost(
        postId: reportId, reason: selectedValue.value?.title ?? '', description: descriptionController.text.trim());
    stopLoader();
  ...
  void _reportComment() async {
    ...
    StatusModel model = await PostService.instance.reportComment(
        commentId: reportId, reason: selectedValue.value?.title ?? '', description: descriptionController.text.trim());
    stopLoader();
    Get.back();
    if (model.status == true) {
      showSnackBar(LKey.reportSubmitted.tr);
  ...
  void _reportStory() async {
    ...
    StatusModel model = await PostService.instance.reportStory(
        storyId: reportId, reason: selectedValue.value?.title ?? '', description: descriptionController.text.trim());
    stopLoader();
    Get.back();
    if (model.status == true) {
      showSnackBar(LKey.reportSubmitted.tr);
  ...
  void _reportUser() async {
    ...
    StatusModel model = await UserService.instance.reportPost(
        userId: reportId, reason: selectedValue.value?.title ?? '', description: descriptionController.text.trim());
    stopLoader();

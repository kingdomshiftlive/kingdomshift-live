...
class RequestWithdrawalScreenController extends BaseController {
  ...
  Rx<TextEditingController> estimatedAmountController = Rx(TextEditingController(text: '0'));
  ...
  void onChanged(String value) {
    ...
    int maxAllowedCoins = myUser.value?.coinWallet?.toInt() ?? 0; // Dynamic max value
    int inputValue = int.tryParse(value) ?? 0;

    if (inputValue > maxAllowedCoins) {
      showSnackBar(LKey.youCanNotEnterMoreThanEtc.trParams({'coin': maxAllowedCoins.toString()}));
      amountController.text = maxAllowedCoins.toString(); // Set max value dynamically
      amountController.selection = TextSelection.fromPosition(
        TextPosition(offset: amountController.text.length),
      );
    } else {
      estimatedAmountController.value.text = '${(settings.value?.coinValue?.toDouble() ?? 0) * inputValue}';
    }
  }
  ...
  void onSubmit() async {
    ...
    int amount = amountController.text.trim().isEmpty ? 0 : int.parse(amountController.text.trim());
    ...
    StatusModel model = await GiftWalletService.instance.submitWithdrawalRequest(
    ...
    myUser.value?.coinWallet = (myUser.value?.coinWallet ?? 0) - int.parse(amountController.text.trim());

...
class SavedPostScreenController extends BaseController {
  ...
  Future<List<Post>> fetchPost() async {
    ...
    List<Post> _post =
        await PostService.instance.fetchSavedPosts(type: PostType.posts, lastItemId: posts.lastOrNull?.postSaveId);
    if (_post.isNotEmpty) {
      posts.addAll(_post);
    }
  ...
  Future<List<Post>> fetchReel() async {
    ...
    List<Post> _post =
        await PostService.instance.fetchSavedPosts(type: PostType.reels, lastItemId: reels.lastOrNull?.postSaveId);
    if (_post.isNotEmpty) {
      reels.addAll(_post);
    }

...
class SearchScreenController extends BaseController {
  ...
  Future<void> searchPosts({bool reset = false}) async {
    ...
    final items = await SearchService.instance
        .searchPost(type: PostType.posts, lastItemId: reset ? null : posts.lastOrNull?.id, keyword: searchKeyword.text);
  ...
  Future<void> searchReels({bool reset = false}) async {
    ...
    final items = await SearchService.instance
        .searchPost(type: PostType.reels, lastItemId: reset ? null : reels.lastOrNull?.id, keyword: searchKeyword.text);
  ...
  Future<void> searchUsers({bool reset = false}) async {
    ...
    List<User> items = await SearchService.instance
        .searchUsers(lastItemId: reset ? null : users.lastOrNull?.id, keyword: searchKeyword.text);
    if (reset) {
      users.clear();
    }
    if (items.isNotEmpty) {
  ...
  Future<void> searchHashTags({bool reset = false}) async {
    ...
    List<Hashtag> items = await SearchService.instance
        .searchHashtags(keyword: searchKeyword.text.trim(), lastItemId: reset ? null : hashtags.lastOrNull?.id);
    if (reset) {
      hashtags.clear();
    }
    if (items.isNotEmpty) {
  ...
  Future<void> searchPlace({bool reset = false}) async {
    ...
    List<Places> items = await CommonService.instance.searchPlace(title: searchKeyword.text.trim());
  ...
  Future<void> fetchNearByLocation({Position? pos}) async {
    ...
    if (position != null) {
      List<Places> _place = await CommonService.instance.searchNearBy(lat: position.latitude, lon: position.longitude);
  ...
  void onHashTagTap(Hashtag hashTag) {
    Get.to(HashtagScreen(hashtag: hashTag.hashtag ?? ''), preventDuplicates: false);
  }
  ...
  void onLocationTap(Places place) {
    ...
    Get.to(LocationScreen(latLng: latLng, placeTitle: place.title), preventDuplicates: false);
  }
}

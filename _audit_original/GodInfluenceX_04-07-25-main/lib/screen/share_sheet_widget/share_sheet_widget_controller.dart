...
class ShareSheetWidgetController extends BaseController {
  ...
  @override
  void onReady() {
    DefaultCacheManager().downloadFile(setting?.watermarkImage?.addBaseURL() ?? '').then((value) {
      waterMarkPath.value = value.file.path;
  ...
  void onUserTap(ChatThread conversation) {
    if (selectedConversation.contains(conversation)) {
      ...
      if (selectedConversation.length >= AppRes.shareChatLimit) {
        return showSnackBar(LKey.shareLimitMessage.trParams({'share_chat_limit': AppRes.shareChatLimit.toString()}));
      }
  ...
  onSearchUser(String value) {
    filterChatsUsers.value = chatsUsers.search(value, (p0) => p0.chatUser?.username ?? '');
  }

  void onShareSheetBottomBtnTap(ShareOption type, String link, {Post? post}) async {
    ...
    Future<void> _handleUrlLaunch(String url, {String? fallbackUrl}) async {
      ...
      } else if (result.message == 'ACTIVITY_NOT_FOUND' && fallbackUrl != null) {
    ...
    switch (type) {
      case ShareOption.whatsapp:
        await _handleUrlLaunch(type.value('$title $link'), fallbackUrl: AppRes.whatsappPlayStoreLink);
        break;

      case ShareOption.instagram:
        if (Platform.isIOS) {
          final result = await type.value('$title $link').lunchUrl;
          Get.back();
          if (result.status == true) {
            increaseShareCount(postId);
          ...
          try {
            final success = await _retrytechPlugin.shareToInstagram('$title $link');
            if (success == true) {
              increaseShareCount(postId);
            }
  ...
  void _downloadReel(Post? reel) async {
    ...
    try {
      // Download video file
      String videoInputPath = (await DefaultCacheManager().downloadFile(videoUrl)).file.path;
  ...
  void onSendChat(Post? post) async {
    ...
    for (var element in controller.selectedConversation) {
      final controller = Get.put(ChatScreenController(element.obs), tag: '${element.conversationId}');
      await controller.sendMessageToFireStore(type: MessageType.post, postMessage: jsonEncode(post?.toJsonForChat()));
  ...
  void increaseShareCount(int? postId) async {
    StatusModel response = await PostService.instance.increaseShareCount(postId: postId);

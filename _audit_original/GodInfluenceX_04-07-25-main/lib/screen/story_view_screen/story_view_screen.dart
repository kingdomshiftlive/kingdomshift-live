...
class StoryViewSheet extends StatelessWidget {
  ...
  const StoryViewSheet({super.key, required this.stories, required this.userIndex, required this.onUpdateDeleteStory});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(
        StoryViewScreenController(stories, userIndex, PageController(initialPage: userIndex), onUpdateDeleteStory));
    return Container(
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              if (item.story?.music != null)
                                Row(
                                  spacing: 5,
                                  children: [
                                    Image.asset(AssetRes.icMusic, width: 12, height: 12),
                                    Expanded(
                                      child: Text(
                        ...
                        Obx(
                          () {
                            ...
                            bool isOwnStory = user?.id == myUser?.id;
                            bool shouldDeleteStory = isOwnStory || isModerator;
                            bool isOtherUserStory = !isOwnStory;
                              ...
                              menuItems.add(MenuItem(
                                ...
                                () {
                                  ...
                                  controller.onStoryDelete(item.story,
                                      isModerator: (!isMyStory && isModerator));
                                },
                              ));
                            }
                              ...
                              menuItems.addAll([
                                MenuItem(
                                  ...
                                  () {
                                    ...
                                    Get.bottomSheet(
                                      ReportSheet(
                                          id: item.story?.id, reportType: ReportType.story),
                                      isScrollControlled: true,
                                    ).then((_) => controller.storyController.play());
                                  },
                                ),
                                MenuItem(
                                  LKey.block.tr,
                                  () {
                                    ...
                                    BlockUserController().blockUser(user, () {
                                      Get.back();
                                    });
                                  },
                                ),
                            ...
                            return CustomPopupMenuButton(
                              ...
                              child: Padding(
                                ...
                                child: Image.asset(AssetRes.icMore1,
                                    height: 25, width: 25, color: whitePure(context)),
                              ),
                            );
                          },
                        ),
                        InkWell(
                          onTap: Get.back,
                          child: Image.asset(AssetRes.icClose1, width: 28, height: 28, color: whitePure(context)),
                        ),
                      ],
                    ),
                  ),

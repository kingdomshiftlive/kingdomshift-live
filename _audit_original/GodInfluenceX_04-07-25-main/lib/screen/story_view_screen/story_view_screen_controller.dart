...
class StoryViewScreenController extends BaseController {
  ...
  StoryViewScreenController(this.users, this.userIndex, this.pageController, this.onUpdateStoryDelete) {
  ...
  void onStoryShow(StoryItem value) async {
    if (!value.viewedByUsersIds.contains('${SessionManager.instance.getUserID()}')) {
      Story? story = await PostService.instance.viewStory(storyId: value.id.toInt());
      if (story != null) {
        if (userIndex < users.length) {
          List<Story> stories = users[userIndex].stories ?? [];
          int storyIndex = stories.indexWhere((element) => element.id == story.id);
          if (storyIndex != -1) {
    ...
    Get.bottomSheet(
      ConfirmationSheet(
        ...
        onTap: () async {
          ...
          StatusModel status;
          if (isModerator) {
            status = await ModeratorService.instance.moderatorDeleteStory(storyId: story?.id ?? -1);
          } else {
            status = await PostService.instance.deleteStory(storyId: story?.id ?? -1);
          }
          stopLoader();
  ...
  void onPreviousUser() {
    ...
    pageController.animateToPage(userIndex - 1, duration: const Duration(milliseconds: 300), curve: Curves.linear);
    update();
  }

              ...
              child: Column(
                children: [
                  GradientText(LKey.plus.tr,
                      gradient: StyleRes.themeGradient, style: TextStyleCustom.unboundedExtraBold800(fontSize: 44)),
                  const SizedBox(height: 10),
                  Text(LKey.subscribeToPlus.tr,
                      style: TextStyleCustom.outFitRegular400(fontSize: 18, color: textLightGrey(context)),
                      textAlign: TextAlign.center),
                  const SizedBox(height: 22),
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      BuildIconWithText(icon: AssetRes.icNoAds, title: LKey.noAds.tr),
                      const SizedBox(width: 10),
                      BuildIconWithText(icon: AssetRes.icBlueTick, title: LKey.getVerified.tr),
                    ],
                  ),
                  Obx(
                    () => Padding(
                      ...
                      child: Column(
                        children: List.generate(
                          ...
                          (index) {
                            ...
                            return Obx(() {
                              bool isSelected = controller.selectedPackage.value?.identifier == package.identifier;
                              return InkWell(
                                onTap: () => controller.onSubscriptionTap(package),
                                child: Container(
                                  ...
                                  decoration: ShapeDecoration(
                                      ...
                                      shadows:
                                          isSelected ? [BoxShadow(color: disableGrey(context), blurRadius: 10)] : null),
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            if (package.getDetail.description.isNotEmpty)
                                              Text(
...
class BuildIconWithText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: ShapeDecoration(
        shape: SmoothRectangleBorder(
            borderRadius: SmoothBorderRadius(cornerRadius: 30), side: BorderSide(color: bgGrey(context))),
...
extension RevenueCatProduct on Package {
  SubscriptionDetail get getDetail {
    ...
    String getTrialDescription() {
      ...
      return LKey.freeTrialDescription.trParams({'count': '$count', 'get_period': getPeriod});
    }
    ...
    String getBilledDescription(String unitLabel, int months) {
      ...
      return trial.isEmpty
          ? months <= 1
              ? ''
              : LKey.subscriptionDescription.trParams({'price': calculatePrice(months), 'unit_label': unitLabel})
          : trial;
    }
    ...
    return switch (packageType) {
      ...
      PackageType.annual =>
        SubscriptionDetail(title: LKey.annual.tr, description: getBilledDescription(LKey.annually.tr, 12)),
      PackageType.sixMonth =>
        SubscriptionDetail(title: LKey.sixMonth.tr, description: getBilledDescription(LKey.semiAnnually.tr, 6)),
      PackageType.threeMonth =>
        SubscriptionDetail(title: LKey.threeMonth.tr, description: getBilledDescription(LKey.threeMonths.tr, 3)),
      PackageType.twoMonth =>
        SubscriptionDetail(title: LKey.twoMonth.tr, description: getBilledDescription(LKey.twoMonths.tr, 2)),
      PackageType.monthly =>
        SubscriptionDetail(title: LKey.monthly.tr, description: getBilledDescription(LKey.monthly.tr, 1)),
      PackageType.weekly => SubscriptionDetail(title: LKey.weekly.tr, description: LKey.giveItATry.tr),
    };
  }

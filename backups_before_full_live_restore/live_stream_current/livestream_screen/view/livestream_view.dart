...
class LivestreamView extends StatelessWidget {
  ...
  const LivestreamView({super.key, required this.streamViews, required this.controller});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      // EMERGENCY FIX: host camera must render directly full-screen.
      // This bypasses profile/gray overlays and grid layouts.
      if (controller.isHost && views.isNotEmpty) {
  ...
  Widget _buildScrollableGrid({
    ...
    final itemHeight = Get.width / 2 / columns; // Each item is square (1:1 aspect ratio)
          ...
          child: Row(
            children: [
              ...
              if (rowUsers.length < columns && rowIndex == rows - 1)
                for (int i = 0; i < columns - rowUsers.length; i++) Expanded(child: _buildEmptyUserView()),
            ],
          ),
        );
      }),
      ...
      child: Row(
        children: [
          ...
          if (streamViews.length < 2) ...[
            for (int i = 0; i < 2 - streamViews.length; i++) Expanded(child: _buildEmptyUserView()),
          ],
        ],
      ),
    );
...
class FourUserView extends StatelessWidget {
  ...
  const FourUserView({super.key, required this.controller, required this.streamViews});
      ...
      child: Row(
        children: [
          for (final user in streamViews.take(2))
            Expanded(
              child: LiveStreamUserView(
                  isNameAndSpeakerVisible: streamViews.toList().indexOf(user) != 0,
                  controller: controller,
                  streamingView: user),
            ),
...
class FiveUserView extends StatelessWidget {
  ...
  const FiveUserView({super.key, required this.controller, required this.streamViews});
      ...
      child: Row(
        children: [
          ...
          if (streamViews.length < 3)
            for (int i = 0; i < 3 - streamViews.length; i++) Expanded(child: _buildEmptyUserView()),
        ],
      ),
    );
  }
...
class SixUserView extends StatelessWidget {
  ...
  const SixUserView({super.key, required this.controller, required this.streamViews});
...
class SevenUserView extends StatelessWidget {
  ...
  const SevenUserView({super.key, required this.controller, required this.streamViews});
      ...
      child: Row(
        children: [
          ...
          if (streamViews.length < 4)
            for (int i = 0; i < 4 - streamViews.length; i++) Expanded(child: _buildEmptyUserView()),
        ],
      ),
    );
  }
...
class EightUserView extends StatelessWidget {
  ...
  const EightUserView({super.key, required this.controller, required this.streamViews});
...
class LiveStreamUserView extends StatelessWidget {
    ...
    return ClipRRect(
      ...
      child: Obx(() {
        LivestreamUserState? state = controller.liveUsersStates
            .firstWhereOrNull((element) => element.userId == int.parse(streamingView?.streamId ?? ''));
        AppUser? liveUser = controller.firestoreController.users
            .firstWhereOrNull((element) => element.userId == int.parse(streamingView?.streamId ?? ''));

        final userId = int.tryParse(streamingView?.streamId ?? '');
        final isMainUser = (controller.liveData.value.mainUserId ?? controller.liveData.value.hostId) == userId;
  ...
  void _showUserActionSheet(AppUser user, Rx<LivestreamUserState?> state) {
    Get.bottomSheet(
      LiveStreamUserInfoSheet(isAudience: !controller.isHost, liveUser: user, controller: controller),

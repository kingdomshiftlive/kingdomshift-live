...
class AudioLiveStreamController extends BaseController {
  ...
  Future<ZegoRoomLoginResult> loginRoom() async {
    ...
    final user = ZegoUser(
        myUserId.toString(), SessionManager.instance.getUser()?.username ?? '');
      ...
      if (!stateSnap.exists) {
        ...
        if (user != null) {
          final initialState = LivestreamUserState(
            type:
                isHost ? LivestreamUserType.host : LivestreamUserType.audience,
            userId: myUserId,
          ...
          if (isHost) {
            ...
            Loggers.info(
                'Host camera disabled and video muted for audio-only stream');
          }
        }
      }
  ...
  void startListenEvent() {
    ZegoExpressEngine.onRoomUserUpdate =
      ...
      Loggers.info(
          'onRoomUserUpdate: roomID: $roomID, updateType: ${updateType.name}');
    };
    ...
    ZegoExpressEngine.onRoomStreamUpdate =
      ...
      if (updateType == ZegoUpdateType.Delete &&
          streamList
              .any((stream) => stream.streamID == liveData.value.roomID)) {
        Get.back();
      }
    };
  }
  ...
  Future<void> startHostPublish() async {
    ...
    Loggers.info(
        'Host publishing audio-only stream: camera disabled, video muted');
  ...
  void listenUserState() {
    liveStreamUserStatesListener = liveStreamUserStatesRef
        .withConverter(
          fromFirestore: (snapshot, options) => snapshot.exists
              ? LivestreamUserState.fromJson(snapshot.data()!)
              : null,
  ...
  void fetchLiveStreamComments() {
    liveStreamCommentsListener = liveStreamCommentsRef
        .withConverter(
          fromFirestore: (snapshot, options) => snapshot.exists
              ? LivestreamComment.fromJson(snapshot.data()!)
              : null,
          toFirestore: (value, options) => value?.toJson() ?? {},
        )
        .snapshots()
  ...
  void onGiftTap({List<AppUser> users = const []}) {
    ...
    List<AppUser> streamUsers = users.isEmpty
        ? liveUsersStates
  ...
  void toggleAudioMute() {
    ...
    VideoAudioStatus newStatus =
        isAudioOn ? VideoAudioStatus.offByMe : VideoAudioStatus.on;
  ...
  Future<void> updateUserStateToFirestore(
    ...
    if (!isExist) {
      Loggers.error(
          'updateUserStateToFirestore: User state not found for userId $userId');
      return;
    }
    ...
    try {
      final updateData = <String, dynamic>{
        ...
        if (followersGained != null)
          FirebaseConst.followersGained: followersGained,
        if (joinStreamTime != null)
          FirebaseConst.joinStreamTime: joinStreamTime,
      };
      ...
      ConfirmationSheet(
        ...
        onTap: () async {
          if (isHost) {
            ...
            zegoEngine.logoutRoom(liveData.value.roomID ?? '');
            Loggers.success(
                'Host ended audio live stream: ${liveData.value.roomID}');
          } else {
            ...
            zegoEngine.logoutRoom(liveData.value.roomID ?? '');
            Loggers.info(
                'Audience left audio live stream: ${liveData.value.roomID}');
          }

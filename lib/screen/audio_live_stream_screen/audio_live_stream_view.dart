...
class AudioLiveStreamView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      AppUser? hostUser = controller.firestoreController.users
          .firstWhereOrNull((user) => user.userId == int.parse(hostId));
      LivestreamUserState? hostState = controller.liveUsersStates
          .firstWhereOrNull((state) => state.userId == int.parse(hostId));
      ...
      return Stack(
        children: [
          ...
          if (controller.isHost || hostUser.userId != controller.myUserId)
  ...
  Widget _buildMuteButton(
      BuildContext context, AudioLiveStreamController controller) {
    return Positioned(
      top: 80,
      left: 10,
      child: Padding(
        ...
        child: Obx(() {
          ...
          final userState = controller.liveUsersStates.firstWhereOrNull(
              (element) => element.userId == controller.myUserId);

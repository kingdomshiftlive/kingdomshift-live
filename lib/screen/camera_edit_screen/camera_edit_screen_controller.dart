...
class CameraEditScreenController extends BaseController {
  ...
  Future<void> addStory(
    ...
    try {
      ...
      final ext = content.split('.').last.toLowerCase();
      final isVideo = type == PostStoryContentType.storyVideo;
      final bucket = isVideo ? 'videos' : 'thumbnails';
      final fileName =
          '${firebaseUser.uid}/story_${DateTime.now().millisecondsSinceEpoch}.$ext';

      await supabase.Supabase.instance.client.storage.from(bucket).upload(
          fileName, file,
          fileOptions: const supabase.FileOptions(upsert: true));

      final contentUrl = supabase.Supabase.instance.client.storage
          .from(bucket)
      ...
      if (thumbnail != null && thumbnail.isNotEmpty) {
        ...
        final thumbName =
            '${firebaseUser.uid}/story_thumb_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      final result = await supabase.Supabase.instance.client
          ...
          .insert({
            ...
            'thumbnail_url': thumbnailUrl ?? contentUrl,
            'type': isVideo ? 1 : 0,
            'duration': duration,
          })
          .select()
      ...
      final story = Story(
        id: result['id'].toString().hashCode,
        userId: 100,
        content: contentUrl,
        thumbnail: thumbnailUrl ?? contentUrl,
        type: isVideo ? 1 : 0,
        duration: duration.toString(),
        createdAt: result['created_at'],
      );

      addStoryResponse(story);
    } catch (e) {
  ...
  Future<void> changeBg(bool isTextStory) async {
    if (isTextStory) {
  ...
  Future<void> changeStoryTime() async {

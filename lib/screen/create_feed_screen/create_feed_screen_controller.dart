...
class CreateFeedScreenController extends BaseController {
  ...
  Future<PostModel?> _handleReelUpload(
    ...
    double progress = 10;
    final finalVideoFile = await _applyFinalVideoCompression(videoPath,
        progressCallback: (double v) {
  ...
  Future<PostModel?> _handleVideoUpload(Map<String, dynamic> params) async {
    ...
    Loggers.info('Applying final video compression...');
    final finalCompressedVideoFile = await _applyFinalVideoCompression(
        finalVideoFile.path, progressCallback: (double v) {
    ...
    // Step 5: Upload video
    Loggers.info('Uploading video...');
    FilePathModel uploadedVideo = await CommonService.instance
        .uploadFileGivePath(finalCompressedVideoFile);
    ...
    try {
      ...
      if (createType == CreateFeedType.podcast) {
        result = await AddPostStoryService.instance
            .addPostFeedPodcast(param: params);
      } else {
        result =
  ...
  Future<XFile?> _applyFinalVideoCompression(String videoPath,
      {dynamic Function(double)? progressCallback}) async {
    try {
      Loggers.info('🎬 Applying final video compression before upload...');

      final compressedFile = await VideoCompressor.compressVideo(videoPath,
      ...
      if (compressedFile != null) {
        ...
        Loggers.warning(
            '⚠️ Final video compression failed, using original file');

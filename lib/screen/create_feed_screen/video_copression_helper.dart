...
class VideoCompressor {
  ...
  static Future<File?> compressVideo(
    ...
    try {
      ...
      logger(
          '📊 File size: ${fileSizeMB.toStringAsFixed(2)} MB ($fileSize bytes)');

      if (fileSizeMB < 2.0) {
        logger(
            '📹 VideoCompressor: File is already small ($fileSizeMB MB), skipping compression');
      ...
      if (result != null) {
        ...
        logger(
            '   💾 Space saved: ${(originalSizeMB - compressedSizeMB).toStringAsFixed(2)} MB');
        logger(
            '   ⏱️ Total processing time: ${DateTime.now().toIso8601String()}');
      } else {
        logger(
            '❌ VideoCompressor: Compression failed - no output file generated');
      }
...
Future<File?> _compressVideo(String inputPath, CompressionQuality quality,
  ...
  try {
    ...
    logger(
        '   - File size: ${(mediaInfo.filesize! / (1024 * 1024)).toStringAsFixed(2)} MB');
    ...
    if (result != null) {
      ...
      if (await compressedFile.exists()) {
        ...
        final compressedMediaInfo =
            await VideoCompress.getMediaInfo(result.path!);
        ...
        logger(
            '      - Size: ${originalSizeMB.toStringAsFixed(2)} MB ($originalSize bytes)');
        logger('      - Path: $inputPath');
        logger('   📤 Compressed file:');
        logger(
            '      - Size: ${compressedSizeMB.toStringAsFixed(2)} MB ($compressedSize bytes)');
        ...
        logger(
            '   🔢 Compression factor: ${(originalSize / compressedSize).toStringAsFixed(2)}x');
        logger(
        ...
        logger('      - Resolution: ${mediaInfo.width}x${mediaInfo.height}');
        logger(
            '      - Duration: ${(mediaInfo.duration! / 1000).toStringAsFixed(2)}s');
        logger('   📤 Compressed video:');
        logger(
            '      - Resolution: ${compressedMediaInfo.width}x${compressedMediaInfo.height}');
        logger(
            '      - Duration: ${(compressedMediaInfo.duration! / 1000).toStringAsFixed(2)}s');
        logger('');

        // Quality preservation analysis
        final resolutionPreserved =
            mediaInfo.width == compressedMediaInfo.width &&
                mediaInfo.height == compressedMediaInfo.height;
        final durationPreserved =
            (mediaInfo.duration! - compressedMediaInfo.duration!).abs() <
                1000; // 1 second tolerance

        logger('✅ QUALITY PRESERVATION:');
        logger(
            '   ${resolutionPreserved ? "✅" : "⚠️"} Resolution: ${resolutionPreserved ? "Preserved" : "Changed"}');
        logger(
            '   ${durationPreserved ? "✅" : "⚠️"} Duration: ${durationPreserved ? "Preserved" : "Changed"}');
        logger('');
        ...
        logger(
            '   📉 File size reduced by: ${compressionRatio.toStringAsFixed(1)}%');
        logger('   💾 Space saved: ${spaceSavedMB.toStringAsFixed(2)} MB');
        logger(
            '   ⏱️ Processing completed at: ${DateTime.now().toIso8601String()}');
        logger('═══════════════════════════════════════════════════════════');

        return compressedFile;
      } else {
        logger(
            '❌ VideoCompressor: Output file not created despite successful result');
        return null;
      }
    } else {

...
class LivestreamScreenController extends BaseController {
  ...
  void onInit() {
    ...
    if (liveData.value.isDummyLive == 1) {
      ...
      totalBattleSecond = Duration(minutes: liveData.value.battleDuration).inSeconds;
    ...
    initAudioPlayer();
    WakelockPlus.enable();
    FirebaseNotificationManager.instance.unsubscribeToTopic(topic: myUserId.toString());
    createMediaPlayer();
  ...
  void onClose() {
    ...
    logoutRoom(4);
    if (!isHost) {
      updateLiveStreamData(watchingCount: -1, coHostId: FieldValue.arrayRemove([myUserId]));
    }
      ...
      if (!stateSnap.exists) {
        ...
        if (state != null) {
          updateUserStateToFirestore(myUserId,
              type: state.type, audioStatus: state.audioStatus, videoStatus: state.videoStatus);
        }
      }
  ...
  RxBool isScreenShareAvailable = false.obs;

  /// Create screen capture source
  Future<void> createScreenCaptureSource() async {
    try {
      // For Android and iOS, sourceId and sourceType are not required
      screenCaptureSource = await ZegoExpressEngine.instance.createScreenCaptureSource();
      isScreenShareAvailable.value = true;
      Loggers.success('Screen capture source created successfully');
    } catch (e) {
      Loggers.error('Failed to create screen capture source: $e');
      isScreenShareAvailable.value = false;
    }
  }

  /// Start screen sharing
  Future<void> startScreenSharing() async {
    bool permissionOk = await requestPermission();
    if (!permissionOk) return;

    if (screenCaptureSource == null) {
      await createScreenCaptureSource();
    }

    if (screenCaptureSource == null) {
      showSnackBar('Failed to create screen sharing source');
      return;
    }

    try {
      // Set video source to screen capture
      await ZegoExpressEngine.instance.setVideoSource(ZegoVideoSourceType.ScreenCapture,
          instanceID: screenCaptureSource!.getIndex(), channel: ZegoPublishChannel.Main);

      // Configure screen capture - CORRECTED: Only 2 parameters
      var config = ZegoScreenCaptureConfig(
          true, // captureVideo
          true // captureAudio
          );

      // Start capture - inApp: false for system-wide sharing
      await screenCaptureSource!.startCapture(config: config, inApp: false);

      isScreenSharing.value = true;

      // Update livestream data to indicate screen sharing
      updateLiveStreamData(type: LivestreamType.livestream);

      // Send comment to notify audience
      _sendCommentToFirestore(type: LivestreamCommentType.text, comment: "Started screen sharing");

      showSnackBar('Screen sharing started successfully');
      Loggers.success('Screen sharing started');
    } catch (e) {
      Loggers.error('Failed to start screen sharing: $e');
      showSnackBar('Failed to start screen sharing: $e');
  ...
  Future<void> stopScreenSharing() async {
    if (screenCaptureSource == null || !isScreenSharing.value) {
      return;
    }
    ...
    try {
      ...
      // Reset video source back to camera
      await ZegoExpressEngine.instance.setVideoSource(ZegoVideoSourceType.Camera, channel: ZegoPublishChannel.Main);

      // Re-enable camera for normal streaming
      await zegoEngine.enableCamera(true);

      isScreenSharing.value = false;

      // Send comment to notify audience
      _sendCommentToFirestore(type: LivestreamCommentType.text, comment: "Stopped screen sharing");

      showSnackBar('Screen sharing stopped');
      Loggers.success('Screen sharing stopped');
    } catch (e) {
      Loggers.error('Failed to stop screen sharing: $e');
      showSnackBar('Failed to stop screen sharing: $e');
    }
  }

  /// Toggle screen sharing
  void toggleScreenSharing() => isScreenSharing.value ? stopScreenSharing() : startScreenSharing();

  /// Screen sharing exception callback - CORRECTED
  void setupScreenSharingCallbacks() {
    // CORRECTED: Use proper callback registration
    ZegoExpressEngine.onExceptionOccurred =
        (ZegoScreenCaptureSource source, ZegoScreenCaptureSourceExceptionType exceptionType) {
      Loggers.error('Screen capture exception: ${exceptionType.name}');
      isScreenSharing.value = false;
      showSnackBar('Screen sharing stopped due to exception');
    };

    // For Android platform specific callbacks - CORRECTED
    ZegoExpressEngine.onMobileScreenCaptureStart = () {
      Loggers.info('Mobile screen capture started');
      isScreenSharing.value = true;
    };

    // CORRECTED: Proper callback assignment
    ZegoExpressEngine.onMobileScreenCaptureExceptionOccurred = (ZegoScreenCaptureExceptionType exceptionType) {
      Loggers.error('Mobile screen capture exception: ${exceptionType.name}');
      isScreenSharing.value = false;
      showSnackBar('Screen sharing failed: ${exceptionType.name}');
    };
  }
  ...
  Future<void> startHostPublish() async {
    ...
    // Ensure camera is enabled for video streaming
    await zegoEngine.enableCamera(true, channel: ZegoPublishChannel.Main);

    // Ensure video and audio are not muted for video live streaming
  ...
  Future<void> startPlayStream(String streamID) async {
    ...
    try {
      await zegoEngine.createCanvasView((viewID) {
        ...
        config.resourceMode = ZegoStreamResourceMode.OnlyL3; // Force L3
  ...
  Future<void> stopPlayStream(String streamID) async {
    ...
    try {
      ...
      StreamView? stream = streamViews.firstWhereOrNull((element) => element.streamId == streamID);
  ...
  Future<void> updateLiveStreamData({
    ...
    liveStreamDocRef.update({
      ...
      if (battleMode != null) FirebaseConst.battleMode: battleMode.value,
      if (type != null) FirebaseConst.type: type.value,
      if (streamLayout != null) FirebaseConst.streamLayout: streamLayout.value,
      if (updateMainUserId) FirebaseConst.mainUserId: mainUserId,
  ...
  void changeStreamLayout(StreamLayout layout) {
    if (!isHost) return;
    updateLiveStreamData(streamLayout: layout);
  }

  void assignMainUser(int? userId) {
    if (!isHost) return;
    updateLiveStreamData(mainUserId: userId, updateMainUserId: true);
  }
  ...
  void handleRequestResponse({
    ...
    updateUserStateToFirestore(userId, type: isRefused ? LivestreamUserType.audience : LivestreamUserType.coHost);

    // Determine the comment to delete
    final commentToDelete = comment ??
        comments.firstWhereOrNull(
            (element) => element.senderId == userId && element.commentType == LivestreamCommentType.request);
  ...
  Future<void> deleteStreamOnFirebase() async {
    ...
    try {
      ...
      Loggers.info('Deleted ${usersSnapshot.docs.length} livestream users_state.');
      ...
      Loggers.info('Deleted ${commentsSnapshot.docs.length} livestream comments.');
      ...
      // Commit batch delete
      await batch.commit();
      Loggers.success('livestream , users_states , comments  deleted from Firestore.');
    } catch (e, stackTrace) {
    ...
    liveStreamDocListener = liveStreamDocRef
        .withConverter<Livestream>(
          fromFirestore: (snapshot, _) {
            if (!snapshot.exists) {
              Loggers.error('Livestream document not found for Room ID: ${liveData.value.roomID}');
      ...
      (event) async {
        ...
        if (stream.battleType == BattleType.initiate) {
          timer?.cancel();
          remainingBattleSeconds.value = Duration(minutes: stream.battleDuration).inSeconds;
  ...
  void listenUserState() {
    ...
    liveStreamUserStatesListener = liveStreamUserStatesRef
        ...
        .listen((event) {
            ...
            switch (change.type) {
              ...
              case DocumentChangeType.modified:
                LivestreamUserState? oldState =
                    liveUsersStates.firstWhereOrNull((element) => element.userId == state.userId);
          ...
          requestList.value = liveUsersStates.where((element) => element.type == LivestreamUserType.requested).toList();
          audienceList.value = liveUsersStates
              .where((element) => element.type != LivestreamUserType.host && element.type != LivestreamUserType.left)
              .toList();
          invitedList.value = liveUsersStates.where((element) => element.type == LivestreamUserType.invited).toList();
          coHostList.value = liveUsersStates.where((element) => element.type == LivestreamUserType.coHost).toList();
          audienceMemberList.value = liveUsersStates
              .where((element) => element.type != LivestreamUserType.left && element.userId != myUserId)
              .toList();
        });
  }
  ...
  void toggleVideo(LivestreamUserState? state) {
    ...
    final isVideoOn = state?.videoStatus == VideoAudioStatus.on;
    if (isVideoOn) {
      updateUserStateToFirestore(myUserId, videoStatus: VideoAudioStatus.offByMe);
      // Stop sending video to network + stop camera capture
      zegoEngine.mutePublishStreamVideo(true, channel: ZegoPublishChannel.Main); // [3]
      zegoEngine.enableCamera(false); // [14]
    } else {
      updateUserStateToFirestore(myUserId, videoStatus: VideoAudioStatus.on);
      zegoEngine.enableCamera(true); // [14]
      zegoEngine.mutePublishStreamVideo(false, channel: ZegoPublishChannel.Main); // [3]
    }
  }
  ...
  //     zegoEngine.mutePublishStreamVideo(true);
  //   } else {
  //     updateUserStateToFirestore(myUserId, videoStatus: VideoAudioStatus.on);
  //     zegoEngine.enableCamera(true);
  //     zegoEngine.mutePublishStreamVideo(false);
  //   }
  // }

  void toggleStreamAudio(int? streamId) {
    StreamView? view = streamViews.firstWhereOrNull((element) => int.parse(element.streamId) == streamId);

    zegoEngine.mutePlayStreamAudio('$streamId', (view?.isMuted ?? false) ? false : true);
    view?.isMuted = view.isMuted ? false : true;
    if (view != null) {
      streamViews[streamViews.indexWhere((element) => int.parse(element.streamId) == streamId)] = view;
      streamViews.refresh();
    }
  }
  ...
  void onGiftTap(GiftType type, {BattleView battleViewType = BattleView.red, List<AppUser> users = const []}) {
    users.removeWhere((element) => element.userId == myUserId);
    if (users.isEmpty) {
      return showSnackBar(LKey.cannotSendGiftToYourself.tr);
    }
    if (liveData.value.type == LivestreamType.battle && liveData.value.battleType == BattleType.end) {
      return showSnackBar(LKey.battleEndedGiftNotSent.tr);
    }
    GiftManager.openGiftSheet(
        onCompletion: (giftManager) async {
          if (giftManager.gift.type == 'video') {
            await loadResource(videoBaseUrl + giftManager.gift.image.toString());
            await playAnim();
          }
          ...
          _sendCommentToFirestore(type: LivestreamCommentType.gift, giftId: gift.id, receiverId: user?.userId);
  ...
  _sendCommentToFirestore({required LivestreamCommentType type, String? comment, int? giftId, int? receiverId}) async {
    int time = DateTime.now().millisecondsSinceEpoch;
    try {
      await _addUsersFirebaseFireStore();
      liveStreamCommentsRef.doc('$time').set(LivestreamComment(
              comment: comment, commentType: type, id: time, senderId: myUserId, receiverId: receiverId, giftId: giftId)
          .toJson());
    } catch (e) {
  ...
  void onVideoRequestSend(Livestream liveData) {
    ...
    switch (state?.type) {
      ...
      case LivestreamUserType.audience:
        updateUserStateToFirestore(myUserId, type: LivestreamUserType.requested);
        _sendCommentToFirestore(type: LivestreamCommentType.request, receiverId: liveData.hostId);
  ...
  Future<void> updateUserStateToFirestore(
    ...
    try {
      final updateData = <String, dynamic>{
        ...
        if (isFollow != null)
          FirebaseConst.followersGained:
              isFollow ? FieldValue.arrayUnion([myUserId]) : FieldValue.arrayRemove([myUserId]),
        if (joinTime != null) FirebaseConst.joinStreamTime: joinTime,
      };
      if (battleCoin != null || liveCoin != null) {
        myUser.value?.coinEstimatedValue(battleCoin?.toDouble() ?? liveCoin?.toDouble());
  ...
  void onInvite(AppUser? user, {bool isInvited = false}) {
    updateUserStateToFirestore(user?.userId,
        type: isInvited ? LivestreamUserType.audience : LivestreamUserType.invited);
  }
              ...
              LiveStreamJoinSheet(
                  ...
                  onJoined: () async {
                    ...
                    if (userState?.type == LivestreamUserType.invited) {
                      updateUserStateToFirestore(myUserId, type: LivestreamUserType.coHost);
                    } else {
                      showSnackBar(LKey.joinCancelledDescription.tr);
                    }
                  },
                  onCancel: () {
                    updateUserStateToFirestore(myUserId, type: LivestreamUserType.audience);
                  }),
  ...
  void publishCoHostStream(int streamId) async {
    ...
    if (isPermissionGranted) {
      ...
      await zegoEngine.createCanvasView((viewID) async {
        canvasViewID = viewID;
        ZegoCanvas previewCanvas = ZegoCanvas(canvasViewID, viewMode: ZegoViewMode.AspectFill);
      ...
      updateUserStateToFirestore(myUserId, joinTime: DateTime.now().millisecondsSinceEpoch);
    } else {
  ...
  Future<bool> _checkZegoPermission(String permissionType) async {
    try {
      ...
      Loggers.info("Zego permission check for $permissionType - skipping and returning true");
      return true;
    } catch (e) {
      // If Zego permission methods don't exist, skip and return true
      Loggers.info("Zego permission method not available for $permissionType - skipping and returning true");
      return true;
    }
  }

  void closeCoHostStream(int? streamId) {
    StreamView? view = streamViews.firstWhereOrNull((element) => element.streamId == '$streamId');
    if (view != null) {
      ...
      LivestreamComment? comment = comments.firstWhereOrNull(
          (element) => element.senderId == myUserId && element.commentType == LivestreamCommentType.joinedCoHost);
      if (comment != null) {
  ...
  void coHostVideoToggle(LivestreamUserState state) {
    ...
    updateUserStateToFirestore(state.userId,
        videoStatus: state.videoStatus == VideoAudioStatus.on ? VideoAudioStatus.offByHost : VideoAudioStatus.on);
  }
  ...
  void coHostAudioToggle(LivestreamUserState state) {
    ...
    updateUserStateToFirestore(state.userId,
        audioStatus: state.audioStatus == VideoAudioStatus.on ? VideoAudioStatus.offByHost : VideoAudioStatus.on);
  }

  void updateStateAction(LivestreamUserState? oldState, LivestreamUserState newState) {
    if (newState.userId == myUserId) {
      Loggers.info('Updating state for userId: ${newState.toJson()}');
      if (newState.type == LivestreamUserType.coHost && oldState?.type != LivestreamUserType.coHost) {
      ...
      if (newState.type == LivestreamUserType.invited && oldState?.type == LivestreamUserType.audience) {
        _showJoinStreamSheet(newState);
      }
      // if (oldState?.videoStatus != newState.videoStatus) {
      //   bool isVideoOn = newState.videoStatus == VideoAudioStatus.on ? false : true;
      //   zegoEngine.enableCamera(isVideoOn);
      //   zegoEngine.mutePublishStreamVideo(!isVideoOn);
      // }
      if (oldState?.videoStatus != newState.videoStatus) {
        final shouldEnable = newState.videoStatus == VideoAudioStatus.on;
        zegoEngine.enableCamera(shouldEnable, channel: ZegoPublishChannel.Main); // [14]
        zegoEngine.mutePublishStreamVideo(!shouldEnable, channel: ZegoPublishChannel.Main); // [3]
      }
      if (oldState?.audioStatus != newState.audioStatus) {
        bool isAudioOn = newState.audioStatus == VideoAudioStatus.on ? false : true;
        zegoEngine.muteMicrophone(isAudioOn);
      }
      if (oldState?.type == LivestreamUserType.coHost && newState.type == LivestreamUserType.audience) {
  ...
  void coHostDelete(LivestreamUserState state) {
    if (state.type == LivestreamUserType.coHost) {
      ...
      updateUserStateToFirestore(state.userId, type: LivestreamUserType.audience);
    }
  }

  void reportUser(int? userId) {
    Get.bottomSheet(ReportSheet(reportType: ReportType.user, id: userId), isScrollControlled: true);
  }
  ...
  void onStopButtonTap() {
    ...
    String title = !isBattleOn ? LKey.endStreamTitle.tr : LKey.stopBattleTitle.tr;
    String description = !isBattleOn ? LKey.endStreamMessage.tr : LKey.stopBattleDescription.tr;
  ...
  void streamEnded() {
    LivestreamUserState? userState = liveUsersStates.firstWhereOrNull((element) => element.userId == myUserId);
    ...
    if (isHost) {
      Get.back();
      Get.off(() => LiveStreamEndScreen(userState: userState, isHost: isHost, viewers: viewers));
    } else {
      if (userState?.type == LivestreamUserType.coHost) {
        Get.bottomSheet(LiveStreamSummary(userState: userState, isHost: isHost, viewers: viewers),
                isScrollControlled: true)
            .then((value) {
          updateUserStateToFirestore(myUserId, battleCoin: 0, liveCoin: 0, currentBattleCoin: 0);
          if ((liveData.value.roomID ?? '').isEmpty) {
            Get.back();
          }
        });
  ...
  void startBattle() {
    ...
    // (1 Host + N Co-hosts)
    final activeUsers =
        liveUsersStates.where((e) => e.type == LivestreamUserType.host || e.type == LivestreamUserType.coHost).toList();

    BattleMode mode = activeUsers.length > 2 ? BattleMode.multiUser : BattleMode.oneVsOne;
  ...
  void battleRunning() {
    ...
    final startTime = DateTime.fromMillisecondsSinceEpoch(stream.battleCreatedAt ?? 0);
    final endTime = startTime.add(Duration(seconds: totalBattleSecond + AppRes.battleStartInSecond));
    ...
    _timerStart(() {
      ...
      if (remainingBattleSeconds.value <= 10) {
        if (!countdownPlayer.playing) {
          countdownPlayer.seek(Duration(seconds: 10 - remainingBattleSeconds.value));
          countdownPlayer.play();
        }
      }

      Loggers.info('[BATTLE RUNNING] Battle end in ${remainingBattleSeconds.value} sec.');
  ...
  void startMinViewerTimeoutCheck() {
    ...
    Loggers.info('Check Min. Viewers Required to continue live $timeoutMinutes Minutes');
  ...
  void pushNotificationToFollowers(Livestream liveData) {
    ...
    NotificationService.instance.pushNotification(
        ...
        title: LKey.liveStreamNotificationTitle.trParams({'name': hostUser?.username ?? ''}),
  ...
  ZegoMediaPlayer? get mediaPlayer => _mediaPlayer;
  ...
  Future<int> loadResource(String url, {ZegoAlphaLayoutType layoutType = ZegoAlphaLayoutType.Left}) async {
    ...
    if (_mediaPlayer != null) {
      // Validate URL
      // if (!url.endsWith('.mp4')) {
      //   debugPrint('Mp4 Player error: Invalid file format, expected .mp4');
      //   return -1;
      // }

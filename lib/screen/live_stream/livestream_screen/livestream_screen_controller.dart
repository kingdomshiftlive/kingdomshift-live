...
class LivestreamScreenController extends BaseController {
  bool _hasStartedHostPublish = false;
  ...
  void onClose() {
    ...
    stopListenEvent();
    logoutRoom(4, deleteForHost: false);
    if (!isHost) {
  ...
  Future<void> logoutRoom(int index, {bool deleteForHost = false}) async {
    if (isHost && deleteForHost) {
      ...
      switch (updateType) {
        case ZegoUpdateType.Add:
          for (final stream in streamList) {
            if (!isHost || stream.streamID != (liveData.value.roomID ?? "")) startPlayStream(stream.streamID);
          }
          break;
        case ZegoUpdateType.Delete:
          for (final stream in streamList) {
            if (liveData.value.roomID == stream.streamID) {
              ...
              // Empty LiveData
              logoutRoom(1, deleteForHost: false);
              stopListenEvent();
  ...
  Future<void> startScreenSharing() async {
    ...
    try {
      ...
      _sendCommentToFirestore(
          type: LivestreamCommentType.text, comment: "Started screen sharing");
  ...
  Future<void> stopScreenSharing() async {
    ...
    try {
      ...
      // Send comment to notify audience
      _sendCommentToFirestore(
          type: LivestreamCommentType.text, comment: "Stopped screen sharing");
  ...
  void toggleScreenSharing() =>
      isScreenSharing.value ? stopScreenSharing() : startScreenSharing();
  ...
  Future<void> startHostPublish() async {
    if (_hasStartedHostPublish) {
      Loggers.info('startHostPublish skipped: already started');
      return;
    }
    _hasStartedHostPublish = true;
    ...
    streamViews.clear();
    streamViews.add(StreamView(streamID, liveData.value.hostViewID ?? -1,
        hostPreview ?? const SizedBox.shrink(), false));
    ...
    await zegoEngine.mutePublishStreamVideo(false,
        channel: ZegoPublishChannel.Main); // Ensure video is not muted
  ...
  void listenLiveStreamData() {
    ...
    liveStreamDocListener = liveStreamDocRef
      ...
      (event) async {
        ...
        // Update LiveData
        if (stream.type != null) liveData.value = stream;
      ...
      for (var change in querySnapshot.docChanges) {
        ...
        switch (change.type) {
          case DocumentChangeType.added:
            ...
            if (isHost &&
                comment.commentType == LivestreamCommentType.gift &&
                comment.giftId != null) {
              Gift? gift =
                  gifts.firstWhereOrNull((gift) => gift.id == comment.giftId);
              if (gift?.type == 'video') {
                loadResource(videoBaseUrl + gift!.image.toString())
                    .then((ret) async {
                  if (ret == 0) {
                    await playAnim();
  ...
  void toggleVideo(LivestreamUserState? state) {
    ...
    if (isVideoOn) {
      ...
      zegoEngine.mutePublishStreamVideo(true,
          channel: ZegoPublishChannel.Main); // [3]
    ...
    GiftManager.openGiftSheet(
        onCompletion: (giftManager) async {
          if (giftManager.gift.type == 'video') {
            await loadResource(
                videoBaseUrl + giftManager.gift.image.toString());
            await playAnim();
          }
  ...
  Future<void> _sendCommentToFirestore(
  ...
  void updateStateAction(
    ...
    if (newState.userId == myUserId) {
      ...
      if (oldState?.videoStatus != newState.videoStatus) {
        ...
        zegoEngine.enableCamera(shouldEnable,
            channel: ZegoPublishChannel.Main); // [14]
        zegoEngine.mutePublishStreamVideo(!shouldEnable,
            channel: ZegoPublishChannel.Main); // [3]
      }
  ...
  Future<void> stopRecording() async {
    ...
    await http.get(Uri.parse('${WebService.post.stopRecording}/$taskId'),
        headers: header);
  }

  void hostEndStream() {
    streamEnded();
    logoutRoom(2, deleteForHost: true);
  }
  ...
  void startBattle() {
    ...
    // (1 Host + N Co-hosts)
    final activeUsers = liveUsersStates
        .where((e) =>
    ...
    BattleMode mode =
        activeUsers.length > 2 ? BattleMode.multiUser : BattleMode.oneVsOne;
  ...
  void startMinViewerTimeoutCheck() {
    if ((minViewerTimeoutTimer?.isActive ?? false) || timeoutMinutes <= 0)
      return;
    Loggers.info(
  ...
  void onCloseAudienceBtn() {
    ...
    Get.bottomSheet(ConfirmationSheet(
        ...
        onTap: () async {
          ...
          logoutRoom(3, deleteForHost: false);
        }));
  }
  ...
  Future<int> loadResource(String url,
    ...
    debugPrint(
        'Mp4 Player loadResource: $url for ${isHost ? 'host' : 'audience'}');
    int ret = -1;
    ...
    if (_mediaPlayer != null) {
      ...
      try {
        ...
        if (response.statusCode != 200) {
          debugPrint(
              'Mp4 Player error: URL not accessible, status: ${response.statusCode}');
          return -1;
        }
      ...
      debugPrint(
          'Mp4 Player error: Media player still null after initialization');
    }
    return ret;
  }

...
class PostScreenController extends BaseController {
  ...
  void _deletePost(Post post, {required bool isModerator}) async {
    ...
    if (isModerator) {
      model =
          await ModeratorService.instance.moderatorDeletePost(postId: post.id);
    } else {
    ...
    BlockUserController().blockUser(post!.user, () {
      if (Get.isRegistered<ProfileScreenController>(
        ...
        final controller =
            Get.find<ProfileScreenController>(tag: ProfileScreenController.tag);
        controller.posts
            .removeWhere((element) => element.userId == post.user!.id);
      }
    });
  }
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
                "You are seeing this post because it is algorithmic trending or you follow the author.",

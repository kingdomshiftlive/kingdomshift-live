...
class ProfileScreenController extends BlockUserController
  ...
  Future<void> fetchPodcast({bool isEmpty = false}) async {
    ...
    try {
      ...
      for (var post in (items?.posts ?? [])) {
        if (podcasts.firstWhereOrNull((element) => element.id == post.id) ==
            null) {
          podcasts.add(post);
        }
      }
  ...
  void onAddPost({Post? post, CreateFeedType? type}) {
    ...
    List<Post> targetList = type == CreateFeedType.feed
        ? posts
        : (type == CreateFeedType.reel ? reels : podcasts);
  ...
  Future<void> onPinUnpinReel(Post post) async {
    if (post.isPinned == 0) {
  ...
  void onDeleteReel(Post post, {required bool isModerator}) {
    Get.bottomSheet(
      ConfirmationSheet(
  ...
  Future<void> updatePinPost(Post post) async {
    List<Post> existingPinPost = [];
  ...
  Future<void> updateUnPinPost(Post post) async {
    StatusModel response = await unpinPost(post);
    if (response.status == true) {
      final controller = Get.find<PostScreenController>(tag: '${post.id}');
      post.isPinned = 0;
